...
#[ffi_export]
pub fn lotrc_read_level_data<'a>(path: char_p::Ref<'a>) -> Option<repr_c::Box<LevelData>> {
    LevelData::read(path.to_str()).ok().map(|x| Box::new(x).into())
}
#[ffi_export]
pub fn lotrc_level_data_free(level_data: repr_c::Box<LevelData>) {
    drop(level_data)
}
#[ffi_export]
pub fn lotrc_level_compressed_data<'a>() -> repr_c::Box<LevelCompressedData<'a>> {
    Box::new(LevelCompressedData::default()).into()
}
#[ffi_export]
pub fn lotrc_level_compressed_data_free(data: repr_c::Box<LevelCompressedData<'_>>) {
    drop(data)
}
#[ffi_export]
pub fn lotrc_level_data_get_version<'a>(src: &'a LevelData) -> Version {
    src.version()
}
