...
use gltf::{
    ...
    json::{
        accessor::ComponentType, buffer::Target, validation::Checked, Accessor, Index, Node, Root,
    },
};
...
use crate::{
    pak::{
        BoundingBox, BufferInfo, HkConstraintData, HkConstraintInfo, IBuffInfo, IndexBuffer,
        LodMeshes, Mat1, Mat2, Mat3, Mat4, MatExtra, ModelInfo, ShapeInfo, VBuffInfo, VertexBuffer,
        VertexUsage,
    },
    types::{dump_bytes, from_bytes, AsData, Crc, Matrix4x4, Version, PC},
};
#[cfg(feature = "python")]
...
#[cfg_attr(feature = "python", pyclass(module = "pak_alt", get_all, set_all))]
...
impl Model {
    pub fn from_data<O: Version + 'static>(
        info: ModelInfo,
        data: &[u8],
        model_data: &HashMap<Crc, Vec<u8>>,
    ) -> Result<Self> {
        let bone_parents: Vec<i32> = from_bytes!(
            O,
            &data[info.bone_parents_offset as usize..],
            info.bones_num as usize
        )?;
        let bones: Vec<Crc> = if info.bones_offset != 0 {
            from_bytes!(
                O,
                &data[info.bones_offset as usize..],
                info.bones_num as usize
            )?
        } else {
            vec![Crc::Key(0); info.bones_num as usize]
        };
        let bone_transforms: Vec<Matrix4x4> = from_bytes!(
            O,
            &data[info.bone_transforms_offset as usize..],
            info.bones_num as usize
        )?;
        let bone_bounding_boxes: Vec<BoundingBox> = from_bytes!(
            O,
            &data[info.bone_bounding_boxes_offset as usize..],
            info.bones_num as usize
        )?;
        let mut mat_order: Vec<u32> =
            from_bytes!(O, &data[info.mat_offset as usize..], info.mat_num as usize)?;
        let mesh_order: Vec<u32> = from_bytes!(
            O,
            &data[info.mesh_order_offset as usize..],
            info.lod3.breakable_end as usize
        )?;
        let mesh_bounding_boxes: Vec<BoundingBox> = from_bytes!(
            O,
            &data[info.mesh_bounding_boxes_offset as usize..],
            info.lod3.breakable_end as usize
        )?;
        let mut vbuff_order: Vec<u32> = from_bytes!(
            O,
            &data[info.vbuff_offset as usize..],
            info.vbuff_num as usize
        )?;
        let mut ibuff_order: Vec<u32> = from_bytes!(
            O,
            &data[info.ibuff_offset as usize..],
            info.ibuff_num as usize
        )?;
        let skin_binds: Vec<Matrix4x4> = from_bytes!(
            O,
            &data[info.skin_binds_offset as usize..],
            info.skin_binds_num as usize
        )?;
        let vals_j: Vec<u32> = from_bytes!(
            O,
            &data[info.vals_j_offset as usize..],
        ...
        let skin_order = if info.skin_order_offset != 0 {
            from_bytes!(
                O,
                &data[info.skin_order_offset as usize..],
                info.skin_binds_num as usize
            )?
        } else {
            Vec::new()
        ...
        let (slots, slot_map) = if info.slots_offset != 0 {
            ...
            let keys2_order = from_bytes!(
                O,
                &data[info.slot_map_offset as usize..],
                keys2.last().unwrap().val as usize
            )?;
            (keys2, keys2_order)
        } else {
            (Vec::new(), Vec::new())
        ...
        let (block_header, block_offsets, blocks) = if info.block_offset != 0 {
            ...
            let block_offsets: Vec<u32> =
                from_bytes!(O, &data[info.block_offset as usize + 4..], n + 1)?;
        ...
        let shapes = (0..info.shape_num as usize)
            .map(|i| {
                from_bytes!(
                    O,
                    Shape,
                    data,
                    info.shape_offset as usize + i * O::size::<ShapeInfo>()
                )
            })
        ...
        let hk_constraint = if info.hk_constraint_offset != 0 {
            Some(from_bytes!(
                ...
                HkConstraint,
                data,
                info.hk_constraint_offset as usize
            )?)
        } else {
            None
        };
        let hk_constraint_datas: Vec<HkConstraintData> = from_bytes!(
            O,
            &data[info.hk_constraint_data_offset as usize..],
            info.hk_constraint_data_num as usize
        )?;

        let mats = HashSet::<u32>::from_iter(mat_order.iter().cloned())
            .into_iter()
            .sorted()
            .collect::<Vec<_>>();
        let mat_map: HashMap<_, _> = mats
            .iter()
            .enumerate()
            .map(|(i, x)| (*x, i as u32))
            .collect();
        mat_order
            .iter_mut()
            .for_each(|x| *x = *mat_map.get(x).unwrap());
        let mats: Vec<Mat> = mats
            .into_iter()
            .map(|off| from_bytes!(O, Mat, &data[off as usize..]))
            .collect::<Result<Vec<_>>>()?;
        let mat_extras: Vec<_> = mats
            .iter()
            .map(|x| {
                Ok(if x.base().mat_extra_offset != 0 {
                    Some(from_bytes!(O, &data[x.base().mat_extra_offset as usize..])?)
                } else {
                    None
                })
            })
        ...
        let vbuffs = HashSet::<u32>::from_iter(vbuff_order.iter().cloned())
            .into_iter()
            .sorted()
            .collect::<Vec<_>>();
        let mut vbuff_map: HashMap<_, _> = vbuffs
            .iter()
            .enumerate()
            .map(|(i, x)| (*x, i as u32))
            .collect();
        vbuff_order
            .iter_mut()
            .for_each(|x| *x = *vbuff_map.get(x).unwrap());
        let vbuffs: Vec<VBuffInfo> = vbuffs
            .into_iter()
            .map(|off| from_bytes!(O, &data[off as usize..]))
            .collect::<Result<Vec<_>>>()?;

        let ibuffs = HashSet::<u32>::from_iter(ibuff_order.iter().cloned())
            .into_iter()
            .sorted()
            .collect::<Vec<_>>();
        let mut ibuff_map: HashMap<_, _> = ibuffs
            .iter()
            .enumerate()
            .map(|(i, x)| (*x, i as u32))
            .collect();
        ibuff_order
            .iter_mut()
            .for_each(|x| *x = *ibuff_map.get(x).unwrap());
        let ibuffs: Vec<IBuffInfo> = ibuffs
            .into_iter()
            .map(|off| from_bytes!(O, &data[off as usize..]))
            .collect::<Result<Vec<_>>>()?;
        let (vertex_data, index_data) = if info.vbuff_num != 0 || info.ibuff_num != 0 {
            ...
            (
                vbuffs
                    .into_iter()
                    .map(|info| from_bytes!(O, VertexBuffer, &buffer[..], info))
                    .collect::<Result<Vec<_>>>()?,
                ibuffs
                    .into_iter()
                    .map(|info| from_bytes!(O, IndexBuffer, &buffer[..], &info))
                    .collect::<Result<Vec<_>>>()?,
            )
        } else {
            (vec![], vec![])
        ...
        let mut buffer_infos: Vec<BufferInfo> = from_bytes!(
            O,
            &data[info.buffer_info_offset as usize..],
            info.mat_num as usize
        )?;
        buffer_infos.iter_mut().for_each(|buff| {
            buff.vbuff_info_offset = *vbuff_map
                .get(&buff.vbuff_info_offset)
                .unwrap_or(&buff.vbuff_info_offset);
            buff.vbuff_info_offset_2 = *vbuff_map
                .get(&buff.vbuff_info_offset_2)
                .unwrap_or(&buff.vbuff_info_offset_2);
            buff.vbuff_info_offset_3 = *vbuff_map
                .get(&buff.vbuff_info_offset_3)
                .unwrap_or(&buff.vbuff_info_offset_3);
            buff.ibuff_info_offset = *ibuff_map
                .get(&buff.ibuff_info_offset)
                .unwrap_or(&buff.ibuff_info_offset);
        });
    ...
    fn dump_mesh_data<O: Version + 'static>(
        &self,
        infos: &mut DumpInfos,
    ) -> (Vec<u8>, Vec<u32>, Vec<u32>) {
        // size, offset, vsize, data, fmt_alt
        let mut tot_size = 0;
        let mut vbuffs_info: Vec<_> = self
            .vertex_data
            .iter()
            .map(|data| {
                ...
                (
                    ...
                    v_size,
                    vals,
                    (data.info.fmt1 & 0x40000) != 0,
                )
            })
            .collect();

        // size, offset, num, data, ty
        let mut ibuffs_info: Vec<_> = self
            .index_data
            .iter()
            .map(|data| {
                let num = data.len();
        ...
        let vbuff_order: Vec<u32> = (0..self.vertex_data.len())
            .map(|x| {
                infos.header.vbuff_info_offset
                    + (O::size::<VBuffInfo>() * (infos.vbuff.len() + x)) as u32
            })
            .collect();
        let mut vbuff_map: HashMap<_, _> = vbuff_order
            .iter()
            .enumerate()
            .map(|(i, x)| (i as u32, *x))
            .collect();
        ...
        let ibuff_order: Vec<u32> = (0..self.index_data.len())
            .map(|x| {
                infos.header.ibuff_info_offset
                    + (O::size::<IBuffInfo>() * (infos.ibuff.len() + x)) as u32
            })
            .collect();
        let mut ibuff_map: HashMap<_, _> = ibuff_order
            .iter()
            .enumerate()
            .map(|(i, x)| (i as u32, *x))
            .collect();
        ...
        let mut ibuffs: Vec<_> = self
            .index_data
            .iter()
            .map(|_| IBuffInfo::default())
            .collect();

        let mut data = Vec::with_capacity(tot_size);
        for buffer_info in &mut buffer_infos {
            let (vbuff_size, offset, v_size, vals, alt) =
                &mut vbuffs_info[buffer_info.vbuff_info_offset as usize];
            if *alt {
            ...
            if buffer_info.vbuff_info_offset_2 != 0xFFFFFFFF {
                let (vbuff_size, offset, v_size, vals, _) =
                    &mut vbuffs_info[buffer_info.vbuff_info_offset_2 as usize];
            ...
            if buffer_info.vbuff_info_offset_3 != 0xFFFFFFFF {
                let (vbuff_size, offset, v_size, vals, _) =
                    &mut vbuffs_info[buffer_info.vbuff_info_offset_3 as usize];
            ...
            if buffer_info.ibuff_info_offset != 0xFFFFFFFF {
                let (_, offset, num, vals, _) =
                    &mut ibuffs_info[buffer_info.ibuff_info_offset as usize];
            ...
            buffer_info.vbuff_info_offset_2 =
                *vbuff_map.get(&buffer_info.vbuff_info_offset_2).unwrap();
            buffer_info.vbuff_info_offset_3 =
                *vbuff_map.get(&buffer_info.vbuff_info_offset_3).unwrap();
    ...
    pub fn dump<O: Version + 'static>(
        ...
        for (i, (mat, mat_extra)) in zip(&self.mats, &self.mat_extras).enumerate() {
            ...
            if let Some(mat_extra) = mat_extra {
                mat.base_mut().mat_extra_offset = infos.header.mat_extra_offset
                    + (O::size::<MatExtra>() * infos.mat_extra.len()) as u32;
            ...
            match mat {
                Mat::Mat1(mat) => {
                    mat_map.insert(
                        i as u32,
                        infos.header.mat1_offset + (O::size::<Mat1>() * infos.mat1.len()) as u32,
                    );
                    infos.mat1.push(mat);
                }
                Mat::Mat2(mat) => {
                    mat_map.insert(
                        i as u32,
                        infos.header.mat2_offset + (O::size::<Mat2>() * infos.mat2.len()) as u32,
                    );
                    infos.mat2.push(mat);
                }
                Mat::Mat3(mat) => {
                    mat_map.insert(
                        i as u32,
                        infos.header.mat3_offset + (O::size::<Mat3>() * infos.mat3.len()) as u32,
                    );
                    infos.mat3.push(mat);
                }
                Mat::Mat4(mat) => {
                    mat_map.insert(
                        i as u32,
                        infos.header.mat4_offset + (O::size::<Mat4>() * infos.mat4.len()) as u32,
                    );
                    infos.mat4.push(mat);
                }
            }
        ...
        info.buffer_info_offset = infos.header.buffer_info_offset
            + (O::size::<BufferInfo>() * (infos.buffer.len() - self.buffer_infos.len())) as u32;
        ...
        info.hk_constraint_data_offset = if self.hk_constraint_datas.len() != 0 {
            infos.header.hk_constraint_data_offset
                + (O::size::<HkConstraintData>() * infos.hk_constraint_data.len()) as u32
        } else {
            0
        };
        infos
            .hk_constraint_data
            .extend(self.hk_constraint_datas.clone());
        ...
        if let Some(hk_constraint) = &self.hk_constraint {
            info.hk_constraint_offset = infos.header.hk_constraint_info_offset
                + (O::size::<HkConstraintInfo>() * infos.hk_constraint.len()) as u32;
            let vals = dump_bytes!(
                O,
                hk_constraint,
                offset,
        ...
        infos
            .block2_offsets
            .extend((0..mat_order.len() as u32).map(|x| offset as u32 + x * 4));
        ...
        infos
            .block2_offsets
            .extend((0..vbuff_order.len() as u32).map(|x| offset as u32 + x * 4));
        ...
        infos
            .block2_offsets
            .extend((0..ibuff_order.len() as u32).map(|x| offset as u32 + x * 4));
    ...
    pub fn dump_terrain<O: Version + 'static>(
        &self,
        mut offset: usize,
        indices_offset: u32,
        infos: &mut DumpInfos,
        ...
        info.bone_bounding_boxes_offset = infos.header.model_info_offset
            + (infos.model.len() * O::size::<ModelInfo>()) as u32
            + 16;
    ...
    pub fn to_gltf(&self) -> Result<gltf::Glb<'_>> {
        ...
        // add index data
        let ibuffs: Vec<_> = self
            .index_data
            .iter()
            .map(|ibuff| {
                match ibuff {
                    IndexBuffer::U16 { vals } => GltfAsset {
                        data: dump_bytes!(PC, vals),
                        count: vals.len(),
                        ty: DataType::U16,
                        dim: Dimensions::Scalar,
                        target: Some(Target::ElementArrayBuffer),
                        ..Default::default()
                    },
                    IndexBuffer::U32 { vals } => GltfAsset {
                        data: dump_bytes!(PC, vals),
                        count: vals.len(),
                        ty: DataType::U32,
                        dim: Dimensions::Scalar,
        ...
        let vbuffs: Vec<(VBuffInfo, HashMap<VertexUsage, Index<Accessor>>)> = self
            .vertex_data
            .iter()
            .map(|vbuff| {
                (
        ...
        let bones: Vec<_> = self
            .bones
            .iter()
            .zip(self.bone_transforms.iter().zip(children))
            .map(|(bone, (mat, children))| {
        ...
        .zip([
            ...
            for i in lod.physics_end..lod.breakable_end {
                uses[(self.mesh_order[i as usize] & 0x3FFFFFFF) as usize] |=
                    f | LodMeshes::BREAKABLE;
            }
            i = lod.breakable_end;
        }
        ...
        let mut nodes: Vec<_> = self
            ...
            .map(|(i, (info, usage))| {
                ...
                for (k, v) in vbuff.iter() {
                    match k {
                        VertexUsage::Position => {
                            attributes
                                .insert(Checked::Valid(gltf::mesh::Semantic::Positions), v.clone());
                        }
                        VertexUsage::Normal => {
                            attributes
                                .insert(Checked::Valid(gltf::mesh::Semantic::Normals), v.clone());
                        }
                        VertexUsage::TextureCoord(i) => {
                            attributes.insert(
                                Checked::Valid(gltf::mesh::Semantic::TexCoords(*i as u32)),
                                v.clone(),
                            );
                        }
                        VertexUsage::BlendIndices => {
                            if skinned {
                                attributes.insert(
                                    Checked::Valid(gltf::mesh::Semantic::Joints(0)),
                                    v.clone(),
                                );
                            } else {
                                attributes.insert(
                                    Checked::Valid(gltf::mesh::Semantic::Extras(k.to_string())),
                                    v.clone(),
                                );
                            }
                        }
                        VertexUsage::BlendWeight => {
                            if skinned {
                                attributes.insert(
                                    Checked::Valid(gltf::mesh::Semantic::Weights(0)),
                                    v.clone(),
                                );
                            } else {
                                attributes.insert(
                        ...
                        VertexUsage::Tangent => {
                            attributes
                                .insert(Checked::Valid(gltf::mesh::Semantic::Tangents), v.clone());
                        }
                        _ => {
                            attributes.insert(
                                Checked::Valid(gltf::mesh::Semantic::Extras(k.to_string())),
                ...
                if usage & LodMeshes::LOD0 != 0 {
                    name += "Lod0, ";
                }
                if usage & LodMeshes::LOD1 != 0 {
                    name += "Lod1, ";
                }
                if usage & LodMeshes::LOD2 != 0 {
                    name += "Lod2, ";
                }
                if usage & LodMeshes::LOD3 != 0 {
                    name += "Lod3, ";
                }
                name.truncate(name.len() - 2);
                name += format!(") Mesh {} [", i).as_str();
                if usage & LodMeshes::UNKNOWN != 0 {
                    name += "Unknown, ";
                }
                if usage & LodMeshes::STATIC != 0 {
                    name += "Static, ";
                }
                if usage & LodMeshes::SKINNED != 0 {
                    name += "Skinned, ";
                }
                if usage & LodMeshes::PHYSICS != 0 {
                    name += "Physics, ";
                }
                if usage & LodMeshes::BREAKABLE != 0 {
                    name += "Breakable, ";
                }
                name.truncate(name.len() - 2);
                name += "]";
        ...
        root.extensions_required
            .push("KHR_mesh_quantization".into());
        root.extras = Some(serde_json::value::to_raw_value(&json!(ModelGltf {
            ...
            shapes: self
                .shapes
                .iter()
                .map(|x| x.to_gltf(&mut root, &mut bin))
                .collect(),
            index_data: ibuffs,
            vertex_data: vbuffs,
            bones,
        ...
        let json = serde_json::to_vec_pretty(&serde_json::from_slice::<serde_json::Value>(
            root.to_vec()?.as_slice(),
        )?)?;
        let length = json.len();
    ...
    pub fn from_gltf(root: &Root, bin: &[u8]) -> Result<Self> {
        ...
        let index_data = model
            .index_data
            .into_iter()
            .map(|i| {
                match root
                    .get::<Accessor>(i.clone())
                    .unwrap()
                    .component_type
                    .as_ref()
                    .unwrap()
                    .0
                {
                    ComponentType::U32 => Ok(IndexBuffer::U32 {
                        vals: GltfData::from_buffer(i, root, bin).u32().unwrap(),
                    }),
                    ComponentType::U16 => Ok(IndexBuffer::U16 {
                        vals: GltfData::from_buffer(i, root, bin).u16().unwrap(),
                    }),
        ...
        let bone_map: HashMap<Index<Node>, usize> = model
            .bones
            .iter()
            .cloned()
            .enumerate()
        ...
        if let Some(skin) = root.skins.first() {
            ...
            skin_binds.extend(
                GltfData::from_buffer(skin.inverse_bind_matrices.unwrap(), root, bin)
                    .f32()
                    .unwrap()
                    .as_slice()
                    .chunks_exact(16)
