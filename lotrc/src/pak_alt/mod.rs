...
use gltf::{
    ...
    json::{
        ...
        buffer::{Buffer, Stride, Target, View},
        validation::Checked,
        Accessor, Index, Root,
    },
};
...
use crate::{
    pak::{
        animation, AnimationInfo, BufferInfo, Header, HkConstraintData, HkConstraintInfo,
        IBuffInfo, Mat1, Mat2, Mat3, Mat4, MatBase, MatExtra, ModelInfo, PFieldInfo,
        RadiosityValsInfo, ShapeInfo, VBuffInfo,
    },
    types::{
        dump_bytes, from_bytes, AsData, BaseTypes, Color, Crc, GameObj, NoArgs, Vector4, Version,
        ANIMATION_EVENTS, PC,
    },
};
...
#[derive(Debug, Default, Clone, Serialize, Deserialize, OrderedData)]
...
impl<'a> AsData<'_, 'a> for HkConstraint {
    ...
    fn from_bytes<O: Version>(data: &[u8], offset: Self::InArgs) -> Result<Self> {
        ...
        if info.kind != 0 {
            panic!(
                "Unknown & Unhandled HkConstraint type {} at offset {}",
                info.kind, offset
            );
        }

        let bone_parents: Vec<i16> = from_bytes!(
            O,
        ...
        let string_offsets: Vec<u32> = from_bytes!(
            O,
        ...
        let bone_transforms = from_bytes!(
            O,
            &data[info.bone_transforms_offset as usize..],
            info.bone_transforms_num as usize
        )?;
        let vals2 = from_bytes!(
            O,
            &data[info.vals2_offset as usize..],
    ...
    fn dump_bytes<O: Version>(
        &self,
        (mut offset, bones_offset, bones_num, infos): Self::OutArgs,
    ) -> Vec<u8> {
        ...
        let bone_order: Vec<_> = self
            .bone_names
            .iter()
            .enumerate()
            .map(|(i, val)| Key2 {
                key: Crc::Str(val.clone()),
                val: i as u32,
            })
            .sorted_by_key(|val| val.key.key())
            .collect();
        ...
        let string_offsets = string_offsets
            .into_iter()
            .flat_map(|x| x)
            .collect::<Vec<_>>();
        ...
        dump_bytes!(O, offsets)
            .into_iter()
            .chain(dump_bytes!(O, string_offsets))
            .chain(data)
            .collect()
    }
...
impl Radiosity {
    pub fn rad_size<V: Version>(&self) -> usize {
        self.vals
            .values()
            .flat_map(|rads| rads.iter().map(|rad| rad.size::<V>()))
            .sum::<usize>()
    }
}
...
impl<'a, 'b> AsData<'a, 'b> for Radiosity {
    type InArgs = (
        u32,
        Vec<RadiosityValsInfo>,
        &'a IndexMap<Crc, Model>,
        &'a IndexMap<u32, GameObj>,
        &'a Vec<u8>,
    );
    type OutArgs = (usize, &'b mut Vec<u8>, &'b mut DumpInfos);

    fn from_bytes<V: Version>(
        data: &[u8],
        (usage, infos, models, objs, rad_data): Self::InArgs,
    ) -> Result<Self> {
        ...
        for info in infos {
            ...
            .expect("level block obj mesh missing");
            let model = models.get(mesh).expect("model missing");
            let sizes: Vec<_> = model
                .buffer_infos
                .iter()
                .map(|x| x.vbuff_size / x.v_size)
                .collect();
            let offsets = from_bytes!(
                V,
                Vec<u32>,
                &data[info.offset as usize..],
                info.num as usize
            )?;
            let rads: Vec<_> = zip(offsets, sizes)
                .map(|(offset, size)| {
                    Ok(if offset != off {
                        RadiosityVal::NoRadiosity(offset)
                    } else {
                        off += size;
                        RadiosityVal::Radiosity(from_bytes!(
                            V,
                            &rad_data[(offset * 4) as usize..],
                            size as usize
                        )?)
                    })
                })
...
impl AsData<'_, '_> for AnimationEvent {
    ...
    fn from_bytes<V: Version>(data: &[u8], _args: Self::InArgs) -> Result<Self> {
        ...
        offset += event.size::<V>();
        let vals = ANIMATION_EVENTS
            .get(&event)
            .iter()
            .flat_map(|x| x.iter().cloned())
            .chain(std::iter::repeat(BaseTypes::INT_KEY))
            .take(9)
            .map(|ty| {
    ...
    fn size<V: Version>(&self) -> usize {
        self.event.size::<V>()
            + self.t.size::<V>()
            + self.vals.iter().map(|x| x.size::<V>()).sum::<usize>()
    }
}
...
impl<'a, 'b> AsData<'a, 'b> for Animation {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        let obj1 = from_bytes!(
            V,
            &data[info.obj1_offset as usize..],
            info.obj1_num as usize * 2
        )?;
        let obj2 = from_bytes!(
            V,
            &data[info.obj2_offset as usize..],
            info.obj2_num as usize
        )?;
        let mut offset = info.obj3_offset as usize;
        let events = (0..info.obj3_num)
            .into_iter()
            .map(|_| {
            ...
            .collect::<Result<Vec<_>>>()?;
        let bones = from_bytes!(
            V,
            &data[info.bones_offset as usize..],
            (info.vals_num + info.obj1_num) as usize
        )?;
        let (obj5_a, obj5_b) = if info.obj5_offset != 0 {
            let obj5_header: animation::Obj5Header =
                from_bytes!(V, &data[info.obj5_offset as usize..])?;
            let obj5_a = from_bytes!(
                V,
                &data[obj5_header.obj_a_offset as usize..],
                obj5_header.obj_a_num as usize
            )?;
            let obj5_b = from_bytes!(
                V,
                &data[obj5_header.obj_b_offset as usize..],
        ...
        let (obj_c3, obj_c4, blocks) = if info.kind == 3 {
            let block_starts: Vec<u32> = from_bytes!(
                V,
                &data[info.block_starts_offset as usize..],
                info.block_starts_num as usize
            )?;
            let block_starts2: Vec<u32> = from_bytes!(
                V,
                &data[info.block_starts2_offset as usize..],
                info.block_starts2_num as usize
            )?;
            let obj_c3 = from_bytes!(
                V,
                &data[info.obj_c3_offset as usize..],
                info.obj_c3_num as usize
            )?;
            let obj_c4 = from_bytes!(
                V,
                &data[info.obj_c4_offset as usize..],
                info.obj_c4_num as usize
            )?;
            ...
            for (start, start2) in zip(block_starts, block_starts2) {
                ...
                let flags: Vec<animation::HkaSplineSkeletalAnimationFlags> =
                    from_bytes!(V, &data[off..], info.vals_num as usize)?;
                let flags2: Vec<u8> =
                    from_bytes!(V, &data[off + flags.size::<V>()..], info.vals2_num as usize)?;
    ...
    fn dump_bytes<V: Version>(&self, (offset, infos): Self::OutArgs) -> Vec<u8> {
        ...
        info.obj2_offset = if self.obj2.is_empty() {
            0
        } else {
            data.len() as u32
        };
        ...
        info.data_offset = ((self.blocks[0].0.len()
            * V::size::<animation::HkaSplineSkeletalAnimationFlags>())
            + self.blocks[0].1.len()) as u32;
        ...
        for (vals, vals2) in &self.blocks {
            ...
            let flags: Vec<_> = vals
                .iter()
                .map(|(a, b, c)| {
                    let f = a.vals.kind() | (b.vals.kind() << 2) | (c.vals.kind() << 6);
                    animation::HkaSplineSkeletalAnimationFlags {
                        a: a.flags,
                        b: b.flags,
                        c: c.flags,
                        f,
                ...
                .collect();
            let flags2: Vec<_> = vals2
                .iter()
                .map(|d| d.flags | (d.vals.kind() << 1))
                .collect();
        ...
        if !self.obj5_a.is_empty() || !self.obj5_b.is_empty() {
            ...
            let header = animation::Obj5Header {
                ...
                obj_b_offset: if self.obj5_b.len() != 0 {
                    (data.len()
                        + V::size::<animation::Obj5Header>()
                        + (self.obj5_a.len() * V::size::<animation::Obj5Val>()))
                        as u32
                } else {
                    0
                },
...
impl Animation {
    pub fn from_data<O: Version + 'static>(
        ...
        let (_, (offset, block)) = zip(offsets.iter().cloned(), blocks.iter())
            .enumerate()
            .find(|(i, _)| info.gamemodemask & (1 << i) != 0)
            .unwrap();
        let val = from_bytes!(O, Animation, &block[offset..], info)?;
        offsets
            .iter_mut()
            .enumerate()
            .filter(|(i, _)| val.info.gamemodemask & (1 << i) != 0)
            .for_each(|(_, x)| *x += val.info.size as usize);
        Ok(val)
    }
    pub fn from_data_alt<O: Version + 'static>(
        info: AnimationInfo,
        offset: &mut usize,
        data: &Vec<u8>,
    ) -> Result<Self> {
        let val = from_bytes!(O, Animation, &data[*offset..], info)?;
        *offset += val.info.size as usize;
        Ok(val)
    }
}
...
impl GltfAsset {
    pub fn to_gltf(self, root: &mut Root, bin: &mut Vec<u8>) -> Index<Accessor> {
        ...
        bin.extend(vec![
            0u8;
            ((bin.len() + s - 1) & (0xFFFFFFFF - s + 1)) - bin.len()
        ]);
        ...
        root.push(gltf::json::Accessor {
            ...
            extras: self
                .extras
                .map(|x| serde_json::value::to_raw_value(&x).unwrap()),
...
impl<'a> GltfData<'a> {
    pub fn from_buffer(i: Index<Accessor>, root: &Root, bin: &'a [u8]) -> Self {
        ...
        let stride = buffer_view
            .byte_stride
            .map(|x| x.0)
            .unwrap_or_else(|| ty.size() * m);
        let off = (buffer_view.byte_offset.map(|x| x.0).unwrap_or(0)
            + accessor.byte_offset.map(|x| x.0).unwrap_or(0)) as usize;
        Self {
            ty,
            m,
            n,
    ...
    pub fn u8(self) -> Result<Vec<u8>> {
        match self.ty {
            DataType::U8 => (0..self.n)
                .map(|x| Vec::<u8>::from_bytes::<PC>(&self.bin[x * self.stride..], self.m))
                .flatten_ok()
                .collect::<Result<Vec<_>>>(),
    ...
    pub fn i8(self) -> Result<Vec<i8>> {
        match self.ty {
            DataType::I8 => (0..self.n)
                .map(|x| Vec::<i8>::from_bytes::<PC>(&self.bin[x * self.stride..], self.m))
                .flatten_ok()
                .collect::<Result<Vec<_>>>(),
    ...
    pub fn u16(self) -> Result<Vec<u16>> {
        match self.ty {
            DataType::U16 => (0..self.n)
                .map(|x| Vec::<u16>::from_bytes::<PC>(&self.bin[x * self.stride..], self.m))
                .flatten_ok()
                .collect::<Result<Vec<_>>>(),
    ...
    pub fn i16(self) -> Result<Vec<i16>> {
        match self.ty {
            DataType::I16 => (0..self.n)
                .map(|x| Vec::<i16>::from_bytes::<PC>(&self.bin[x * self.stride..], self.m))
                .flatten_ok()
                .collect::<Result<Vec<_>>>(),
    ...
    pub fn u32(self) -> Result<Vec<u32>> {
        match self.ty {
            DataType::U32 => (0..self.n)
                .map(|x| Vec::<u32>::from_bytes::<PC>(&self.bin[x * self.stride..], self.m))
                .flatten_ok()
                .collect::<Result<Vec<_>>>(),
    ...
    pub fn f32(self) -> Result<Vec<f32>> {
        match self.ty {
            DataType::F32 => (0..self.n)
                .map(|x| Vec::<f32>::from_bytes::<PC>(&self.bin[x * self.stride..], self.m))
                .flatten_ok()
                .collect::<Result<Vec<_>>>(),
