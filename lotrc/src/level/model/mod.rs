...
use crate::{
    types::{Crc, DumpData, Matrix4x4, RefFromData, Vector3, DumpSlice, align_offset, CompressedDataRef},
    level::{
...
use crate::{
    level::{
        model::{
            mat::DumpMatVER,
            shape::{DumpShapeVER, DumpHkConstraintVER, ShapeVER, HkShapeVER, HkConstraintDataVER, DumpShapeExtraVER},
            data::ModelDataVER,
        },
        pak::{
    ...
    types::{CrcVER, Matrix4x4VER, I32VER, U16VER, U32VER},
};
...
pub fn init(py: Python<'_>) -> PyResult<Bound<'_, PyModule>> {
    let m = PyModule::new(py, "model")?;
    m.add_class::<BlockHeader>()?;
    m.add_class::<BlockVal>()?;
    m.add_class::<BoundingBox>()?;
    m.add_class::<LodInfo>()?;
    m.add_class::<Model>()?;
    m.add_class::<ModelInfo>()?;
    m.add_submodule(&data::init(py)?)?;
    m.add_submodule(&mat::init(py)?)?;
    m.add_submodule(&shape::init(py)?)?;
    init_pc(&m)?;
    init_xbox(&m)?;
    init_ps3(&m)?;
    Ok(m)
...
pub struct BonesRefVER<'a> {
    names:  &'a [CrcVER],
    parents: &'a [I32VER],
    transforms: &'a [Matrix4x4VER],
    bounding_boxes: &'a [BoundingBoxVER],
}

#[make_platforms]
impl BonesRefVER<'_> {
    pub fn names(&self) -> &[CrcVER] {
        self.names
    }
    pub fn parents(&self) -> &[I32VER] {
        self.parents
    }
    pub fn transforms(&self) -> &[Matrix4x4VER] {
        self.transforms
    }
    pub fn bounding_boxes(&self) -> &[BoundingBoxVER] {
        self.bounding_boxes
    }
}
...
#[make_platforms]
impl BonesVER {
    pub fn from_bytes(src: &Arc<[u8]>, info: &ModelInfoVER) -> Result<Self> {
    ...
    pub unsafe fn as_ref(&self) -> BonesRefVER<'_> {
        BonesRefVER {
            names: self.names.map(|x| unsafe { NonNull::slice_from_raw_parts(x, self.size).as_ref() }).unwrap_or(&[]),
            parents: self.parents.map(|x| unsafe { NonNull::slice_from_raw_parts(x, self.size).as_ref() }).unwrap_or(&[]),
            transforms: self.transforms.map(|x| unsafe { NonNull::slice_from_raw_parts(x, self.size).as_ref() }).unwrap_or(&[]),
            bounding_boxes: self.bounding_boxes.map(|x| unsafe { NonNull::slice_from_raw_parts(x, self.size).as_ref() }).unwrap_or(&[]),
        }
    }
}
...
#[cfg(feature = "python")]
#[make_platforms]
impl<'a, 'py> IntoPyObject<'py> for BonesRefVER<'a> {
    type Target = <Bones as IntoPyObject<'py>>::Target;
    type Output = <Bones as IntoPyObject<'py>>::Output;
    type Error = <Bones as IntoPyObject<'py>>::Error;
    #[inline(always)]
    fn into_pyobject(self, py: Python<'py>) -> Result<Self::Output, Self::Error> {
        Bones::from(self).into_pyobject(py)
    }
}
...
pub struct ModelRawVER {
    ...
    blocks: Box<[BlockVER]>,
    val: NonNull<[U32VER]>,
    buffer_infos: NonNull<[data::BufferInfoVER]>,
    vbuff_order: NonNull<[U32VER]>,
    ibuff_order: NonNull<[U32VER]>,
    vbuffs: IndexMap<u32, NonNull<data::VBuffInfoVER>>,
    ibuffs: IndexMap<u32, NonNull<data::IBuffInfoVER>>,
...
#[make_platforms]
impl ModelRawVER {
    pub fn from_bytes(src: &Arc<[u8]>, model_data: &IndexMap<u32, CompressedDataRef>, offset: usize) -> Result<Self> {
        ...
        let vbuffs = vbuff_order.iter().map(|x| Ok((x.get(), NonNull::from_ref(data::VBuffInfoVER::from_data(&src[x.get() as usize..]).with_context(|| format!("vbuff info {}", x.get()))?)))).collect::<Result<_>>()?;
        let ibuffs = ibuff_order.iter().map(|x| Ok((x.get(), NonNull::from_ref(data::IBuffInfoVER::from_data(&src[x.get() as usize..]).with_context(|| format!("ibuff info {}", x.get()))?)))).collect::<Result<_>>()?;
        let data = model_data.get(&info.asset_key.get()).cloned().unwrap_or_else(|| CompressedDataRef::empty());
        Ok(Self {
            _ptr: src.clone(),
            info: info.into(),
            bones,
...
pub trait DumpModelVER {
    ...
    fn write_mat_order(&self, mat_order: &mut [U32VER]) -> Result<()>;
    ...
    fn hk_constraint_datas(&self) -> impl Iterator<Item=impl Into<HkConstraintDataVER>>;
    ...
    fn dump_misc(&self, dst: &mut DumpSlice, infos: &mut DumpInfosVER, info: &mut ModelInfoVER) -> Result<()> {
        if self.has_vals_k() {
            dst.align(16);
            info.vals_k_offset = dst.offset.into();
        ...
        info.hk_constraint_data_num = self.hk_constraint_data_num().into();
        if info.hk_constraint_data_num.get() != 0 {
            info.hk_constraint_data_offset = infos.hk_constraint_datas.offset.into();
            for info in self.hk_constraint_datas() {
                *infos.hk_constraint_datas.next() = info.into();
            }
        }
        Ok(())
    }
    ...
    fn dump_into(&self, dst: &mut DumpSlice, infos: &mut DumpInfosVER) -> Result<()> {
        ...
        info.shape_num = self.shape_num().into();
        if info.shape_num.get() != 0 {
            info.shape_offset = infos.shapes.offset.into();
        ...
        info.mesh_order_offset = dst.offset.into();
        ...
        dst.align(16);
        info.mesh_bounding_boxes_offset = dst.offset.into();
        ...
        info.vbuff_offset = dst.offset.into();
        let mut off = dst.offset;
        ...
        info.vbuff_num = vbuff_order.len().into();
        self.write_vbuff_order(vbuff_order).context("write vbuff_order")?;

        info.ibuff_offset = dst.offset.into();
        let mut off = dst.offset;
        ...
        info.ibuff_num = ibuff_order.len().into();
    ...
    fn dump_terrain_into(&self, dst: &mut DumpSlice, infos: &mut DumpInfosVER, indices_offset: usize) -> Result<()> {
        ...
        info.bones_num = self.bone_num().into();
        info.bone_bounding_boxes_offset = bone_bbox_offset.into();
        info.skin_binds_offset = indices_offset.into();
        info.skin_binds_num = 0u32.into();
        info.skin_order_offset = 0u32.into();
        info.bone_parents_offset = indices_offset.into();
