...
use serde_json::{from_slice, json, to_vec_pretty, Map, Value};
use std::any::TypeId;
use std::io::prelude::*;
use std::sync::Mutex;
use std::{
...
use zerocopy::{
    ByteOrder, FromBytes, Immutable, IntoBytes, KnownLayout, Unaligned, BE, F32, I16, I32, LE, U16,
    U32, U64,
};
...
pub trait PyMethods<'a>
where
    Self: std::fmt::Debug + Sized + Deserialize<'a>,
{
...
pub struct XBOX;
pub struct PS3;
pub struct XBOXPROTO;
pub struct XBOXPROTO2;
...
impl Version for PC {
    ...
    #[inline]
    fn from_bytes_vec<T: OrderedData>(data: &[u8], num: usize) -> Result<Vec<T>> {
        Ok(<[T::PC]>::ref_from_prefix_with_elems(data, num)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .iter()
            .map(|x| x.clone().into())
...
impl Version for XBOX {
    ...
    #[inline]
    fn from_bytes<T: OrderedData>(data: &[u8]) -> Result<T> {
        Ok(T::XBOX::read_from_prefix(data)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .into())
    }
    ...
    #[inline]
    fn from_bytes_vec<T: OrderedData>(data: &[u8], num: usize) -> Result<Vec<T>> {
        Ok(<[T::XBOX]>::ref_from_prefix_with_elems(data, num)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .iter()
            .map(|x| x.clone().into())
...
impl Version for PS3 {
    ...
    #[inline]
    fn from_bytes<T: OrderedData>(data: &[u8]) -> Result<T> {
        Ok(T::PS3::read_from_prefix(data)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .into())
    }
    ...
    #[inline]
    fn from_bytes_vec<T: OrderedData>(data: &[u8], num: usize) -> Result<Vec<T>> {
        Ok(<[T::PS3]>::ref_from_prefix_with_elems(data, num)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .iter()
            .map(|x| x.clone().into())
...
impl Version for XBOXPROTO {
    ...
    #[inline]
    fn from_bytes<T: OrderedData>(data: &[u8]) -> Result<T> {
        Ok(T::XBOXPROTO::read_from_prefix(data)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .into())
    }
    #[inline]
    fn to_bytes<T: OrderedData>(val: &T, data: &mut [u8]) -> Result<()> {
        T::XBOXPROTO::write_to_prefix(&val.clone().into(), data).map_err(|e| anyhow!("{:?}", e))?;
        Ok(())
    }
    #[inline]
    fn dump_bytes<T: OrderedData>(val: &T) -> Vec<u8> {
        T::XBOXPROTO::as_bytes(&val.clone().into()).to_vec()
    }
    #[inline]
    fn size<T: OrderedData>() -> usize {
        size_of::<T::XBOXPROTO>()
    ...
    #[inline]
    fn from_bytes_vec<T: OrderedData>(data: &[u8], num: usize) -> Result<Vec<T>> {
        Ok(<[T::XBOXPROTO]>::ref_from_prefix_with_elems(data, num)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .iter()
            .map(|x| x.clone().into())
...
impl Version for XBOXPROTO2 {
    ...
    #[inline]
    fn from_bytes<T: OrderedData>(data: &[u8]) -> Result<T> {
        Ok(T::XBOXPROTO2::read_from_prefix(data)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .into())
    }
    #[inline]
    fn to_bytes<T: OrderedData>(val: &T, data: &mut [u8]) -> Result<()> {
        T::XBOXPROTO2::write_to_prefix(&val.clone().into(), data)
            .map_err(|e| anyhow!("{:?}", e))?;
        Ok(())
    }
    #[inline]
    fn dump_bytes<T: OrderedData>(val: &T) -> Vec<u8> {
        T::XBOXPROTO2::as_bytes(&val.clone().into()).to_vec()
    }
    #[inline]
    fn size<T: OrderedData>() -> usize {
        size_of::<T::XBOXPROTO2>()
    }
    #[inline]
    fn from_bytes_vec<T: OrderedData>(data: &[u8], num: usize) -> Result<Vec<T>> {
        Ok(<[T::XBOXPROTO2]>::ref_from_prefix_with_elems(data, num)
            .map_err(|e| anyhow!("{}", e))?
            .0
            .iter()
            .map(|x| x.clone().into())
...
impl OrderedData for f32 {
    type PC = F32<LE>;
    type XBOX = F32<BE>;
    type PS3 = F32<BE>;
    type XBOXPROTO = F32<BE>;
    type XBOXPROTO2 = F32<BE>;
}
impl OrderedData for u64 {
    type PC = U64<LE>;
    type XBOX = U64<BE>;
    type PS3 = U64<BE>;
    type XBOXPROTO = U64<BE>;
    type XBOXPROTO2 = U64<BE>;
}
impl OrderedData for u32 {
    type PC = U32<LE>;
    type XBOX = U32<BE>;
    type PS3 = U32<BE>;
    type XBOXPROTO = U32<BE>;
    type XBOXPROTO2 = U32<BE>;
}
impl OrderedData for i32 {
    type PC = I32<LE>;
    type XBOX = I32<BE>;
    type PS3 = I32<BE>;
    type XBOXPROTO = I32<BE>;
    type XBOXPROTO2 = I32<BE>;
}
impl OrderedData for u16 {
    type PC = U16<LE>;
    type XBOX = U16<BE>;
    type PS3 = U16<BE>;
    type XBOXPROTO = U16<BE>;
    type XBOXPROTO2 = U16<BE>;
}
impl OrderedData for i16 {
    type PC = I16<LE>;
    type XBOX = I16<BE>;
    type PS3 = I16<BE>;
    type XBOXPROTO = I16<BE>;
    type XBOXPROTO2 = I16<BE>;
}
impl OrderedData for u8 {
    type PC = u8;
    type XBOX = u8;
    type PS3 = u8;
    type XBOXPROTO = u8;
...
impl<T: OrderedData> AsData<'_, '_> for Vec<T> {
    ...
    type OutArgs = NoArgs;
    #[inline]
    fn from_bytes<V: Version>(data: &[u8], num: usize) -> Result<Self> {
        V::from_bytes_vec(data, num)
    }
    #[inline]
    fn to_bytes<V: Version>(&self, data: &mut [u8], _args: NoArgs) -> Result<()> {
        V::to_bytes_vec(self, data)
    }
    #[inline]
    fn dump_bytes<V: Version>(&self, _args: NoArgs) -> Vec<u8> {
        V::dump_bytes_vec(self)
    }
    #[inline]
    fn size<V: Version>(&self) -> usize {
        V::size_vec(self)
...
impl<T: OrderedData> AsData<'_, '_> for T {
    ...
    type OutArgs = NoArgs;
    #[inline]
    fn from_bytes<V: Version>(data: &[u8], _args: NoArgs) -> Result<Self> {
        V::from_bytes(data)
    }
    #[inline]
    fn to_bytes<V: Version>(&self, data: &mut [u8], _args: NoArgs) -> Result<()> {
        V::to_bytes(self, data)
    }
    #[inline]
    fn dump_bytes<V: Version>(&self, _args: NoArgs) -> Vec<u8> {
        V::dump_bytes(self)
...
#[cfg_attr(feature = "python", pyo3(signature = (string, mask=None)))]
...
#[cfg_attr(feature = "python", pyfunction)]
#[cfg_attr(feature = "python", pyo3(signature = (val=None)))]
...
#[cfg_attr(feature = "python", pyfunction)]
#[cfg_attr(feature = "python", pyo3(name = "zip", signature = (val=None)))]
...
impl Crc {
    ...
    pub fn from_key(val: u32) -> Self {
        match STRING_LOOKUP
            .lock()
            .ok()
            .and_then(|m| m.0.get(&val).map(|x| x.clone()))
        {
            ...
            match string_lookup.0.get_full(&val) {
                Some((ind, _, string)) if ind >= string_lookup.1 => {
                    ADDED_CRCS
                        .lock()
                        .ok()
                        .map(|mut x| x.insert(val, string.clone()));
                }
                None => match value {
                    Crc::Str(string) => {
...
impl AsData<'_, '_> for Strings {
    ...
    fn dump_bytes<V: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        self.strings
            .iter()
            .flat_map(|string| {
                dump_bytes!(V, (string.len() as u32))
                    .into_iter()
                    .chain(string.as_bytes().iter().cloned())
                    .collect::<Vec<_>>()
            })
            .collect()
    }
...
impl Strings {
    ...
    pub fn to_file(&self, writer: Writer) -> Result<()> {
        writer
            .with_extension("json")
            .write(&to_vec_pretty(&json!(self.strings))?)?;
        Ok(())
    }

    pub fn from_file(reader: Reader) -> Result<Self> {
        let name = reader.path().display().to_string();
        let strings = from_slice::<Vec<String>>(&reader.with_extension("json").read()?)
            .context(format!("{}", name))?;
        Ok(Self { strings })
    }
}
...
impl CompressedBlock {
    pub fn from_data<O: Version + 'static>(
        ...
        let data = match size_comp {
            ...
            _ => {
                if TypeId::of::<O>() == TypeId::of::<PS3>() {
                    let mut out = Vec::with_capacity(size);
                    let data = &data[offset..offset + size_comp];
                    let mut s = 2;
                    while s < size_comp {
                        let mut decoder =
                            ZlibDecoder::new_with_decompress(&data[s..], Decompress::new(false));
                    ...
                    let mut out = Vec::with_capacity(size);
                    ZlibDecoder::new(&data[offset..offset + size_comp])
                        .read_to_end(&mut out)
                        .unwrap();
                    out
                }
    ...
    pub fn dump(&self, zero_alt: bool) -> Result<Vec<u8>> {
        ...
        Ok(if c.level() == 0 && zero_alt {
            ...
            while data.len() > 0xffffff {
                if c.level() == 9 {
                    return Err(anyhow!(
                        "Bin Asset could not be compressed to a small enough size"
                    ));
                }
...
impl From<&Matrix4x4> for [f32; 16] {
    fn from(mat: &Matrix4x4) -> Self {
        [
            mat.x.x, mat.x.y, mat.x.z, mat.x.w, mat.y.x, mat.y.y, mat.y.z, mat.y.w, mat.z.x,
            mat.z.y, mat.z.z, mat.z.w, mat.w.x, mat.w.y, mat.w.z, mat.w.w,
        ]
    }
}
...
impl BaseTypes {
    ...
    pub fn from_data<O: Version>(data: &[u8], kind: u32) -> Result<Self> {
        Ok(match kind {
            ...
            Self::STRING_KEY => {
                ...
                let vals = from_bytes!(
                    O,
                    &data[val.offset as usize + val.size::<O>()..],
                    val.num as usize
                )?;
            ...
            Self::STRINGLIST_KEY => {
                ...
                let valss = vals
                    ...
                    .enumerate()
                    .map(|(i, v)| {
                        Ok(String::from_utf8(from_bytes!(
                            O,
                            &data[val.offset as usize
                                + val.size::<O>() * (i + 2)
                                + v.offset as usize..],
                            v.num as usize
                        )?)
                ...
                *off += vals.len() * O::size::<List>();
                for v in vals {
                    to_bytes!(
                        ...
                        List {
                            num: v.len() as u16,
                            offset: (*off - off_ - O::size::<List>()) as u16
                        },
                        data
                    )?;
        ...
        match self {
            ...
            Self::String(vals) => {
                to_bytes!(
                    ...
                    List {
                        num: vals.len() as u16,
                        offset: (data.len() - off - O::size::<List>()) as u16
                    },
                    &mut data[off..]
                )?;
            ...
            Self::StringList(vals) => {
                ...
                for v in vals {
                    to_bytes!(
                        ...
                        List {
                            num: v.len() as u16,
                            offset: (data.len() - off_ - O::size::<List>()) as u16
                        },
                        &mut data[off_..]
                    )?;
    ...
    pub fn off_size<O: Version>(&self) -> usize {
        match self {
            ...
            Self::ObjectList(vals) => vals.size::<O>(),
            Self::NodeList(vals) => vals.size::<O>(),
            Self::IntList(vals) => vals.size::<O>(),
            Self::CRCList(vals) => vals.size::<O>(),
            Self::WeightList(vals) => vals.size::<O>(),
            Self::MatrixList(vals) => vals.size::<O>(),
            _ => 0,
        }
    }
    ...
    pub fn to_json(&self) -> Value {
        match self {
            ...
            Self::NodeList(vals) => json!(vals
                .iter()
                .map(|x| json!([x.x, x.y, x.z, x.w]))
                .collect::<Vec<_>>()),
            Self::IntList(vals) => json!(vals.iter().map(|x| json!(x)).collect::<Vec<_>>()),
            Self::CRCList(vals) => json!(vals
                .iter()
                .map(|x| json!(x.to_string()))
                .collect::<Vec<_>>()),
            Self::WeightList(vals) => json!(vals
                .iter()
                .map(|x| json!([x.x, x.a, x.b, x.c, x.d]))
                .collect::<Vec<_>>()),
            Self::MatrixList(vals) => json!(vals
                .iter()
                .map(|val| json!([
                    val.x.x, val.x.y, val.x.z, val.x.w, val.y.x, val.y.y, val.y.z, val.y.w,
                    val.z.x, val.z.y, val.z.z, val.z.w, val.w.x, val.w.y, val.w.z, val.w.w,
                ]))
                .collect::<Vec<_>>()),
        }
    }
    ...
    pub fn from_json(val: Value, kind: u32) -> Result<Self> {
        Ok(match kind {
            ...
            Self::COLOR_KEY => Self::Color(Color::try_from(
                serde_json::from_value::<String>(val)?.as_str(),
            )?),
            Self::VECTOR2_KEY => Self::Vector2(serde_json::from_value::<(f32, f32)>(val)?.into()),
            Self::VECTOR3_KEY => {
                Self::Vector3(serde_json::from_value::<(f32, f32, f32)>(val)?.into())
            }
            Self::VECTOR4_KEY => {
                Self::Vector4(serde_json::from_value::<(f32, f32, f32, f32)>(val)?.into())
            }
            Self::MATRIX4X4_KEY => {
                Self::Matrix4x4((&serde_json::from_value::<[f32; 16]>(val)?).into())
            }
            ...
            Self::NODELIST_KEY => Self::NodeList(
                serde_json::from_value::<Vec<(f32, f32, f32, f32)>>(val)?
                    .into_iter()
                    .map(|x| x.into())
                    .collect(),
            ),
            Self::INTLISTS_KEY => Self::IntList(serde_json::from_value::<Vec<i32>>(val)?),
            Self::CRCLIST_KEY => Self::CRCList(
                serde_json::from_value::<Vec<String>>(val)?
                    .into_iter()
                    .map(|x| Crc::from_string(&x))
                    .collect(),
            ),
            Self::WEIGHTLIST_KEY => Self::WeightList(
                serde_json::from_value::<Vec<(u32, u8, u8, u8, u8)>>(val)?
                    .into_iter()
                    .map(|x| x.into())
                    .collect(),
            ),
            Self::MATRIXLIST_KEY => Self::MatrixList(
                serde_json::from_value::<Vec<[f32; 16]>>(val)?
                    .iter()
                    .map(|x| x.into())
                    .collect(),
            ),
...
impl AsData<'_, '_> for SubBlock {
    ...
    fn from_bytes<V: Version>(data: &[u8], (key, size): Self::InArgs) -> Result<Self> {
        Ok(match key.key() {
            LangStrings::KEY_POLISH
            | LangStrings::KEY_GERMAN
            | LangStrings::KEY_FRENCH
            | LangStrings::KEY_SPANISH
            | LangStrings::KEY_RUSSIAN
            | LangStrings::KEY_SWEDISH
            | LangStrings::KEY_ENGLISH
            | LangStrings::KEY_ITALIAN
            | LangStrings::KEY_NORWEGIAN => {
                SubBlock::LangStrings(from_bytes!(V, LangStrings, data, size)?)
            }
            ...
            AtlasUV::KEY1 | AtlasUV::KEY2 => {
                SubBlock::AtlasUV(from_bytes!(V, AtlasUV, data, size)?)
            }
            _ => match key.str() {
                Some(x) if x.ends_with(".lua") => {
                    SubBlock::Lua(from_bytes!(V, Lua, data, size, x.to_string())?)
                }
...
impl SubBlock {
    ...
    pub fn from_file(reader: Reader, key: &Crc, keys: &StringKeys) -> Result<Self> {
        Ok(match key.key() {
            ...
            GameObjs::KEY => {
                SubBlock::GameObjs(GameObjs::from_file(reader.with_file_name("level.json"))?)
            }
...
impl<'py> FromPyObject<'py> for SubBlocks {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        Ok(Self {
            blocks: IndexMap::extract_bound(ob)?,
        })
    }
}
...
impl AsData<'_, '_> for SubBlocks {
    ...
    fn from_bytes<V: Version>(data: &[u8], prog: Self::InArgs) -> Result<Self> {
        ...
        prog.inner
            .as_ref()
            .map(|x| x.set_length(header.block_num as u64));
        let block_headers: Vec<SubBlocksBlockHeader> = from_bytes!(
            V,
            &data[V::size::<SubBlocksHeader>()..],
            header.block_num as usize
        )?;
        let blocks = block_headers
            .into_iter()
            .map(|info| {
                prog.inner.as_ref().map(|x| {
                    x.inc(1);
    ...
    fn dump_bytes<V: Version>(&self, prog: Self::OutArgs) -> Vec<u8> {
        prog.inner
            .as_ref()
            .map(|x| x.set_length(self.blocks.len() as u64));
        let header = SubBlocksHeader {
            block_num: self.blocks.len() as u32,
            ..Default::default()
        };
        let mut block_headers = Vec::with_capacity(self.blocks.len());
        let mut offset =
            V::size::<SubBlocksHeader>() + V::size::<SubBlocksBlockHeader>() * self.blocks.len();
        let mut data = vec![];
...
impl SubBlocks {
    pub fn to_file(
        ...
        writer.join("index.json").write(&to_vec_pretty(
    ...
    pub fn from_file(
        ...
        let keys = from_slice::<Vec<Crc>>(&reader.join("index.json").read()?)
            .context(format!("{}/index.json", reader.path().display()))?;
        ...
        let blocks = keys
            ...
            .map(|key| {
                ...
                let block =
                    SubBlock::from_file(reader.join(key.str().unwrap()), &key, string_keys)?;
                Ok((key, block))
            })
            .collect::<Result<_>>()?;
        prog.map(|x| x.finish());
...
impl AsData<'_, '_> for StringKeys {
    ...
    fn dump_bytes<V: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        ...
        dump_bytes!(V, header)
            .into_iter()
            .chain(dump_bytes!(V, vals).into_iter())
            .chain(std::iter::repeat_n(0u8, self.vals.len() * V::size::<u32>()))
            .collect()
    }
...
impl AsData<'_, '_> for LangStrings {
    ...
    fn from_bytes<V: Version>(data: &[u8], size: Self::InArgs) -> Result<Self> {
        ...
        while offset < size {
            ...
            val.strings
                .push(String::from_utf16(string.as_slice()).unwrap());
            offset += 2;
        }
        Ok(val)
    }
    fn dump_bytes<V: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        let vals = self
            .strings
            .iter()
            .flat_map(|x| x.encode_utf16().chain([0u16]))
            .collect::<Vec<_>>();
        dump_bytes!(V, vals)
    }
    fn size<V: Version>(&self) -> usize {
        self.strings
            .iter()
            .map(|x| x.encode_utf16().map(|_| 2).sum::<usize>() + 2)
            .sum::<usize>()
    }
}
...
impl LangStrings {
    ...
    pub fn to_file(&self, writer: Writer, keys: &StringKeys) -> Result<()> {
        let vals = zip(&keys.vals, &self.strings)
            .map(|(key, string)| (key.clone(), string.clone()))
            .collect::<IndexMap<_, _>>();
        writer
            .with_extension("json")
            .write(&to_vec_pretty(&vals)?)?;
        Ok(())
    }
    ...
    pub fn from_file(reader: Reader, keys: &StringKeys) -> Result<Self> {
        let name = reader.path().display().to_string();
        let unordered_vals =
            from_slice::<HashMap<Crc, String>>(&reader.with_extension("json").read()?)
                .context(format!("{}.json", name))?;
...
#[cfg_attr(feature = "python", basicpymethods)]
#[cfg_attr(feature = "python", pyclass(module = "types", set_all, get_all))]
#[derive(Debug, Default, Clone, OrderedData, Serialize, Deserialize)]
#[cfg_attr(feature = "python", derive(PyMethods))]
pub struct SSAVal {
    pub t_start: f32,
...
impl AsData<'_, '_> for SSA {
    ...
    fn size<V: Version>(&self) -> usize {
        self.strings
            .iter()
            .map(|x| x.encode_utf16().map(|_| 2).sum::<usize>())
            .sum::<usize>()
            + 4
            + (PC::size::<SSAVal>() * self.vals.len())
    }
}
...
impl SSA {
    ...
    pub fn from_file(reader: Reader) -> Result<Self> {
        from_slice(&reader.with_extension("json").read()?)
            .with_context(|| anyhow!("{}.json", reader.path().display()))
    }
}
    ...
    fn from_bytes<V: Version>(data: &[u8], (size, name): Self::InArgs) -> Result<Self> {
        let data = if size >= 4 && data[..4] == [27, 76, 117, 97] {
            if *DECOMP_LUA.lock().unwrap() {
                lua::decomp(&data[..size], UNLUAC.lock().unwrap().clone())
                    .context(name.clone())?
                    .as_bytes()
                    .to_vec()
            ...
            if *RECOMP_LUA.lock().unwrap() {
                lua::compile(&data[..size], &name)
                    .context(name.clone())
                    .with_context(|| {
                        String::from_utf8(data.to_vec()).unwrap_or("Non utf8 shader".to_string())
                    })?
            } else {
                (&data[..size]).to_vec()
            }
...
impl Lua {
    ...
    pub fn from_file(reader: Reader) -> Result<Self> {
        ...
        let data = if size >= 4 && data[..4] == [27, 76, 117, 97] {
            if *DECOMP_LUA.lock().unwrap() {
                lua::decomp(&data, UNLUAC.lock().unwrap().clone())
                    .context(format!("{}", name_path))?
                    .as_bytes()
                    .to_vec()
            } else if size >= 11 && data[5..11] != [0, 1, 4, 4, 4, 4] {
...
impl AsData<'_, '_> for GameObjs {
    ...
    fn from_bytes<V: Version>(data: &[u8], gamemodemask: Self::InArgs) -> Result<Self> {
        ...
        for _ in 0..header.types_num {
            ...
            offset += info.size::<V>();
            let fields: Vec<GameObjsTypeField> =
                from_bytes!(V, &data[offset..], info.size as usize)?;
        ...
        for _ in 0..header.obj_num {
            ...
            for t in order.iter().map(|i| &ts[*i]) {
                let val =
                    BaseTypes::from_data::<V>(&data[offset + t.offset as usize..], t.kind.key())?;
    ...
    fn dump_bytes<V: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        ...
        type_data.extend(vec![
            0u8;
            ((type_data.len() + 15) & 0xFFFFFFF0) - type_data.len()
        ]);
        let mut data = dump_bytes!(
            V,
            GameObjsHeader {
                const_: 1296123652,
        ...
        for obj in self.objs.values() {
            ...
            for t in ts {
                ...
                if t.kind.key() == BaseTypes::INTLISTS_KEY {
                    obj_vals.extend(vec![
                        0u8;
                        ((obj_vals.len() + 15) & 0xFFFFFFF0) - obj_vals.len()
                    ]);
                }
            }
            obj_vals.extend(vec![
    ...
    fn size<V: Version>(&self) -> usize {
        let size = ((self.types.len() * V::size::<GameObjsTypeHeader>())
            + (self.types.values().map(|x| x.len() as usize).sum::<usize>()
                * V::size::<GameObjsTypeField>())
            + V::size::<GameObjsHeader>()
            + 15)
            & 0xFFFFFFF0;
        size + self
            .obj_headers::<V>()
            ...
            .sum::<usize>()
            + self.objs.len() * V::size::<GameObjsObjHeader>()
    }
}
...
impl GameObjs {
    ...
    pub fn from_file_old(reader: Reader) -> Result<Self> {
        ...
        let mut val = from_slice::<Value>(&reader.with_extension("json").read()?)
            .with_context(|| format!("{}", name))?;
        let ts = val
            .get("types")
            .ok_or(anyhow!("{} missing types", name))?
            .as_array()
            .ok_or(anyhow!("{} types is not array", name))?;
        let mut types = IndexMap::with_capacity(ts.len());
        for (i, t) in ts.into_iter().enumerate() {
            let ty_name = t
                .get("name")
                .ok_or(anyhow!("{} type {} missing name", name, i))?
                .as_str()
                .ok_or(anyhow!("{} type {} name is not string", name, i))?;
            types.insert(
                Crc::from_string(ty_name),
                t.get("fields")
                    .ok_or(anyhow!("{} type {} missing fields", name, ty_name))?
                    .as_array()
                    .ok_or(anyhow!("{} type {} fields is not array", name, ty_name))?
                    .iter()
                    .enumerate()
                    .map(|(i, v)| {
                        let f_name = v
                        ...
                        Ok(GameObjsTypeField {
                            ...
                            kind: Crc::from_string(
                                v.get("type")
                                    .ok_or(anyhow!(
                                        "{} type {} field {} missing kind",
                                        name,
                                        ty_name,
                                        f_name
                                    ))?
        ...
        let types_order: HashMap<_, _> = types
            .iter()
            .map(|(key, fields)| {
                let mut order: Vec<_> = (0..fields.len()).collect();
                order.sort_by_key(|x| fields[*x].offset);
        ...
        let os = val
            .get_mut("objs")
            .ok_or(anyhow!("{} missing objs", name))?
            .as_array_mut()
            .ok_or(anyhow!("{} obs is not array", name))?;
        let mut objs = IndexMap::with_capacity(os.len());

        for (i, o) in os.into_iter().enumerate() {
            let o_name = o
                .get("type")
                .ok_or(anyhow!("{} obj index {} missing type", name, i))?
                .as_str()
                .ok_or(anyhow!("{} obj index {} type is not string", name, i))?;
            let key = Crc::from_string(o_name);
            let order =
                types_order
                    .get(&key)
                    .ok_or(anyhow!("{} type {} not in types", name, o_name))?;
            let mut o_: HashMap<_, _> = o
                .get_mut("fields")
                .ok_or(anyhow!("{} obj index {} missing fields", name, i))?
                .as_object_mut()
                .ok_or(anyhow!("{} obj index {} fields not map", name, i))?
                .into_iter()
                .map(|(k, v)| (Crc::from_string(k), v.take()))
                .collect();
            let fields: IndexMap<Crc, BaseTypes> = order
                .iter()
                .map(|t| {
                    Ok((
                        t.key.clone(),
                        BaseTypes::from_json(
                            o_.remove(&t.key).ok_or_else(|| {
                                anyhow!(
                                    "{} obj index {} missing field {}",
                                    name,
                                    i,
                                    t.key.to_string()
                                )
                        ...
                        .with_context(|| {
                            format!("{} obj index {} field {}", name, i, t.key.to_string())
                        })?,
                    ))
                })
                .collect::<Result<_>>()?;
            let guid = if let BaseTypes::GUID(val) = fields
                .get(&Crc::Key(3482846511))
                .ok_or(anyhow!("{} obj index {} missing guid field", name, i))?
            {
                *val
            } else {
            ...
            objs.insert(
                ...
                GameObj {
                    layer: o
                        .get("layer")
                        .ok_or(anyhow!("{} obj {} missing layer", name, guid))?
                        .as_u64()
                        .and_then(|x| u32::try_from(x).ok())
                        .ok_or(anyhow!("{} obj {} layer is not u32", name, guid))?,
                    key,
                    fields,
                },
            );
    ...
    pub fn from_file_new(reader: Reader) -> Result<Self> {
        ...
        let types = serde_json::from_value::<IndexMap<Crc, Vec<GameObjsTypeField>>>(
            val.get("types")
                .ok_or(anyhow!("{} missing types", name))?
                .clone(),
        )
        .with_context(|| format!("{} types", name))?;
        ...
        let os = val
            ...
            .ok_or(anyhow!("{} missing objs", name))?
            .as_object_mut()
            .ok_or(anyhow!("{} obs is not map", name))?;
        let mut objs = IndexMap::with_capacity(os.len());
        ...
        for (guid, o) in os.into_iter() {
            ...
            let o_name = o
                .get("type")
                .ok_or(anyhow!("{} obj {} missing type", name, guid))?
                .as_str()
                .ok_or(anyhow!("{} obj {} type is not string", name, guid))?;
            let key = Crc::from_string(o_name);
            let order =
                types_order
                    .get(&key)
            ...
            let mut o_: HashMap<_, _> = o
                .get_mut("fields")
                .ok_or(anyhow!("{} obj {} missing fields", name, guid))?
                .as_object_mut()
                .ok_or(anyhow!("{} obj {} fields not map", name, guid))?
                .into_iter()
                .map(|(k, v)| (Crc::from_string(k), v.take()))
                .collect();
            let fields: IndexMap<Crc, BaseTypes> = order
                ...
                .map(|t| {
                    Ok((
                        ...
                        if t.key.key() == 3482846511 {
                            ...
                            .with_context(|| {
                                format!("{} obj {} field {}", name, guid, t.key.to_string())
                            })?
                        },
                    ))
                })
            ...
            let guid = if let BaseTypes::GUID(val) = fields
                .get(&Crc::Key(3482846511))
                .ok_or(anyhow!("{} obj index {} missing guid field", name, guid))?
            {
                *val
            } else {
                panic!("Parsing BaseType returned incorrect type")
...
impl AsData<'_, '_> for Spray {
    ...
    fn dump_bytes<O: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        dump_bytes!(O, (self.instances.len() as u32))
            .into_iter()
...
impl Spray {
    ...
    pub fn from_file(reader: Reader) -> Result<Self> {
        from_slice(&reader.with_extension("json").read()?)
            .with_context(|| format!("{}.json", reader.path().display()))
    }
}
...
#[cfg_attr(feature = "python", basicpymethods(no_bytes))]
#[derive(Debug, Default, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "python", derive(PyMethods))]
#[cfg_attr(feature = "python", pyclass(module = "types", get_all, set_all))]
...
pub enum PFields {
    ...
    RawData(#[serde_as(as = "serde_with::hex::Hex")] Vec<u8>),
}
