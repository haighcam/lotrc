...
impl<'a, 'b> AsData<'a, 'b> for HkaSplineSkeletalAnimation {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        for (start, start2) in zip(&val.block_starts, &val.block_starts2) {
            ...
            let flags: Vec<HkaSplineSkeletalAnimationFlags> =
            ...
            for flag in &flags {
                let a = from_bytes!(
                    V,
                    HkaSplineSkeletalAnimationObj1,
                    &data[off..],
                    flag.a,
                    flag.f & 3
                )?;
                ...
                let b = from_bytes!(
                    V,
                    HkaSplineSkeletalAnimationObj2,
                    &data[off..],
                    flag.b,
                    (flag.f >> 2) & 0xf
                )?;
    ...
    fn to_bytes<V: Version>(&self, data: &mut [u8], info: Self::OutArgs) -> Result<()> {
        to_bytes!(
            V,
            self.block_starts,
            &mut data[info.block_starts_offset as usize..]
        )?;
        to_bytes!(
            V,
            self.block_starts2,
            &mut data[info.block_starts2_offset as usize..]
        )?;
        to_bytes!(V, self.obj_c3, &mut data[info.obj_c3_offset as usize..])?;
        to_bytes!(V, self.obj_c4, &mut data[info.obj_c4_offset as usize..])?;
        for ((start, start2), ((vals_a, vals_b), (vals_c, vals_d))) in zip(
            zip(&self.block_starts, &self.block_starts2),
            zip(
                zip(&self.vals_a, &self.vals_b),
                zip(&self.vals_c, &self.vals_d),
            ),
        ) {
            let flags: Vec<_> = zip(vals_a, zip(vals_b, vals_c))
                .map(|(a, (b, c))| {
                    let f = a.vals.kind() | (b.vals.kind() << 2) | (c.vals.kind() << 6);
                    HkaSplineSkeletalAnimationFlags {
                        a: a.flags,
                        b: b.flags,
                        c: c.flags,
                        f,
            ...
            to_bytes!(
                V,
                flags2,
                &mut data[(start + info.block_offset) as usize + flags.size::<V>()..]
            )?;
            let mut off = (info.block_offset + start + info.data_offset) as usize;
...
impl<'a, 'b> AsData<'a, 'b> for Animation {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        let obj3 = from_bytes!(
            V,
            &data[info.obj3_offset as usize..],
    ...
    fn to_bytes<V: Version>(&self, data: &mut [u8], info: Self::OutArgs) -> Result<()> {
        ...
        if let Some(obj5_header) = &self.obj5_header {
            ...
            to_bytes!(
                V,
                self.obj5_a,
                &mut data[obj5_header.obj_a_offset as usize..]
            )?;
            to_bytes!(
                V,
                self.obj5_b,
                &mut data[obj5_header.obj_b_offset as usize..]
            )?;
        }
...
impl Animation {
    pub fn unpack_block<O: Version + 'static>(
        anims: &mut [HashMap<usize, Self>],
        infos: &[AnimationInfo],
        data: &[u8],
        offset: usize,
        index: usize,
    ...
    pub fn pack_block<O: Version + 'static>(
        anims: &[HashMap<usize, Self>],
        infos: &[AnimationInfo],
        data: &mut [u8],
        offset: usize,
        index: usize,
    ...
    pub fn unpack_block_proto<O: Version + 'static>(
        anims: &mut [HashMap<usize, Self>],
        infos: &[AnimationInfo],
        data: &[u8],
        num_gamemodes: usize,
        ...
        let mut offset = 0;
        for (anim, info) in zip(anims, infos) {
            let animation: Animation = from_bytes!(O, &data[offset..], info)?;
            offset += info.size as usize;
            for index in 0..num_gamemodes {
                if info.gamemodemask != 0 {
                    anim.insert(index, animation.clone());
                }
            }
        }
        Ok(())
