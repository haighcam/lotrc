...
use std::sync::Mutex;
use std::{
    collections::HashMap, fmt::Display, iter::zip, num::ParseIntError, ops::Div, str::FromStr,
};

use crate::{
    pak_alt::GltfData,
    types::{
        dump_bytes, from_bytes, to_bytes, AsData, BaseTypes, Color, Crc, Matrix4x4, NoArgs,
        Vector2, Vector3, Vector4, Version, PC,
    },
};
...
pub struct VBuffInfo {
    ...
    #[name_xbox(fmt2)]
    #[name_xboxproto(fmt2)]
    #[name_xboxproto2(fmt2)]
    #[name_ps3(unk_9)]
    pub fmt1: u32,
    #[name_xbox(fmt1)]
    #[name_xboxproto(fmt1)]
    #[name_xboxproto2(fmt1)]
    #[name_ps3(size)]
    pub fmt2: u32,
    pub unk_6: u32,
    #[name_ps3(offset)]
...
pub mod model {
    use super::*;
    #[cfg_attr(feature = "python", basicpymethods)]
    #[cfg_attr(feature = "python", pyclass(module = "pak.model", get_all, set_all))]
    #[derive(Debug, Default, Clone, OrderedData, Serialize, Deserialize)]
    #[cfg_attr(feature = "python", derive(PyMethods))]
...
impl<'a, 'b> AsData<'a, 'b> for Model {
    ...
    fn from_bytes<O: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        val.indices = from_bytes!(
            O,
            &data[info.bone_parents_offset as usize..],
            info.bones_num.max(4) as usize
        )?;
        ...
        if info.slots_offset != 0 {
            ...
            val.keys2_order = from_bytes!(
                O,
                &data[info.slot_map_offset as usize..],
                *val.keys2.last().unwrap() as usize
            )?;
        }
        if info.block_offset != 0 {
            ...
            for i in 0..n {
                ...
                let vals_a: Vec<u32> =
                    from_bytes!(O, &data[offset + s..], (header.a + header.b) as usize * 12)?;
                s += vals_a.size::<O>();
                let vals_b: Vec<model::BlockVal> = from_bytes!(
                    O,
                    &data[offset + s..],
                    (size - s) / O::size::<model::BlockVal>()
                )?;
                s += vals_b.size::<O>();
        ...
        if (info.mesh_order_offset == info.vbuff_offset)
            && (info.mesh_order_offset == info.ibuff_offset)
            && (info.mesh_order_offset == info.mesh_bounding_boxes_offset)
        {
    ...
    fn to_bytes<O: Version>(&self, data: &mut [u8], info: Self::OutArgs) -> Result<()> {
        to_bytes!(
            O,
            self.indices,
            &mut data[info.bone_parents_offset as usize..]
        )?;
        to_bytes!(O, self.keys, &mut data[info.bones_offset as usize..])?;
        to_bytes!(
            O,
            self.matrices,
            &mut data[info.bone_transforms_offset as usize..]
        )?;
        to_bytes!(
            O,
            self.vals_a,
            &mut data[info.bone_bounding_boxes_offset as usize..]
        )?;
        ...
        to_bytes!(
            O,
            self.vals_d,
            &mut data[info.mesh_bounding_boxes_offset as usize..]
        )?;
            ...
            && (info.vals_j_offset != info.skin_binds_offset)
        {
            ...
            for (v, (off, string)) in zip(&self.vals_j, zip(&self.string_offsets, &self.strings)) {
                ...
                data[*off as usize..*off as usize + string.len()]
        ...
        if info.vals_k_offset != 0 {
            to_bytes!(
                O,
                self.val_k_header,
                &mut data[info.vals_k_offset as usize..]
            )?;
        ...
        if info.slots_offset != 0 {
            ...
            to_bytes!(
                O,
                self.keys2_order,
                &mut data[info.slot_map_offset as usize..]
            )?;
        }
        if info.block_offset != 0 {
            to_bytes!(
                O,
                self.block_header,
                &mut data[info.block_offset as usize..]
            )?;
            to_bytes!(
                O,
                self.block_offsets,
                &mut data[info.block_offset as usize + 4..]
            )?;
...
pub mod shape {
    use super::*;
    #[cfg_attr(feature = "python", basicpymethods)]
    #[cfg_attr(feature = "python", pyclass(module = "pak.shape", get_all, set_all))]
    #[derive(Debug, Default, Clone, OrderedData, Serialize, Deserialize)]
    #[cfg_attr(feature = "python", derive(PyMethods))]
    pub struct Header {
        pub num: i32,
...
#[cfg_attr(feature = "python", basicpymethods)]
#[cfg_attr(feature = "python", pyclass(module = "pak", set_all, get_all))]
#[derive(Debug, Default, Clone, Serialize, Deserialize)]
#[cfg_attr(feature = "python", derive(PyMethods))]
pub struct Shape {
    ...
    fn from_bytes<O: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        if info.kind == 0 {
            ...
            if val.header.num != -1 {
                ...
                val.data = from_bytes!(O, &data[offset..], *val.vals.last().unwrap() as usize + 2)?;
                // might need to be more than +2, not sure
            }
        }
        Ok(val)
    }
...
impl<'a, 'b> AsData<'a, 'b> for HkShape {
    ...
    fn from_bytes<O: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        if info.kind == 5 {
            ...
            while (info.b_offset as usize + b_num * 12) % 16 != 0 {
                b_num += 1;
            }
...
impl<'a, 'b> AsData<'a, 'b> for HkConstraint {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        val.string_offsets = from_bytes!(
            V,
            &data[info.bone_names_offset as usize..],
        ...
        val.vals = from_bytes!(
            V,
            &data[info.bone_transforms_offset as usize..],
            info.bone_transforms_num as usize * 12
        )?;
        val.keys = from_bytes!(
            V,
            &data[info.bones_offset as usize..],
        ...
        val.keys2 = from_bytes!(
            V,
            &data[info.bone_order_offset as usize..],
            info.bone_order_num as usize * 2
        )?;
        Ok(val)
    }
    ...
    fn to_bytes<V: Version>(&self, data: &mut [u8], info: Self::OutArgs) -> Result<()> {
        ...
        to_bytes!(
            V,
            self.string_offsets,
            &mut data[info.bone_names_offset as usize..]
        )?;
        for (offset_, (string, offset, val)) in zip(&self.string_offsets, &self.strings) {
            ...
            data[*offset as usize..*offset as usize + string.len()]
                .copy_from_slice(string.as_bytes());
        }
        to_bytes!(
            V,
...
impl VertexTypes {
    ...
    pub fn get(&self, i: usize) -> BaseTypes {
        match self {
            ...
            Self::Vector3(x, y, z) => BaseTypes::Vector3(Vector3 {
                x: x[i],
                y: y[i],
                z: z[i],
            }),
            Self::Vector4(x, y, z, w) => BaseTypes::Vector4(Vector4 {
                x: x[i],
                y: y[i],
                z: z[i],
                w: w[i],
    ...
    pub fn push(&mut self, val: BaseTypes) {
        match self {
            Self::Pad(vals) => {
                if let BaseTypes::Color(val) = val {
                    vals.push(val.0);
                } else {
                    panic!("VertexTypes Mismatch")
                }
            }
            Self::Unorm4x8(vals) => {
                if let BaseTypes::Color(val) = val {
            ...
            Self::Vector2(x, y) => {
                if let BaseTypes::Vector2(val) = val {
                    x.push(val.x);
                    y.push(val.y);
                } else {
                    panic!("VertexTypes Mismatch")
            ...
            Self::Vector3(x, y, z) => {
                if let BaseTypes::Vector3(val) = val {
                    x.push(val.x);
                    y.push(val.y);
                    z.push(val.z);
                } else {
            ...
            Self::Vector4(x, y, z, w) => {
                if let BaseTypes::Vector4(val) = val {
                    x.push(val.x);
                    y.push(val.y);
                    z.push(val.z);
                    w.push(val.w);
...
impl VertexData {
    ...
    pub fn dump_bytes<O: Version + 'static>(&self) -> Vec<u8> {
        match self {
            Self {
                ...
                val: VertexTypes::Unorm4x8(vals),
            } => dump_bytes!(
                O,
                vals.iter()
                    .map(|x| ((x & 0xFF0000) >> 16) | ((x & 0xFF) << 16) | (x & 0xFF00FF00))
                    .collect::<Vec<_>>()
            ),
            Self {
                val: VertexTypes::Pad(vals),
                ..
            } => dump_bytes!(O, vals),
            Self {
                val: VertexTypes::Unorm4x8(vals),
            ...
            Self {
                val: VertexTypes::Vector4(x, y, z, w),
                ..
            } => dump_bytes!(
                O,
                x.iter()
                    .zip(y.iter().zip(z.iter().zip(w)))
                    .map(|(&x, (&y, (&z, &w)))| Vector4 { x, y, z, w })
                    .collect::<Vec<_>>()
            ),
            Self {
                val: VertexTypes::None(),
                ..
            } => vec![],
        }
    }
    ...
    pub fn gltf_data(&self) -> Vec<u8> {
        ...
        match self {
            Self {
                usage: VertexUsage::Normal,
                val: VertexTypes::Unorm4x8(vals),
            } => {
                dump_bytes!(
                    PC,
                    vals.iter()
                        .map(|x| conv_val((x >> 24) & 0xff) << 24
    ...
    pub fn from_gltf(&mut self, mut data: GltfData) -> Result<(), VertexDataParseError> {
        ...
        match self {
            ...
            Self {
                usage: VertexUsage::Normal | VertexUsage::Position,
                val: VertexTypes::Vector4(x, y, z, w),
            } => {
    ...
    pub fn component_type(&self) -> gltf::accessor::DataType {
        ...
        match self {
            ...
            Self {
                val: VertexTypes::Vector2(..),
                ..
            } => DataType::F32,
            Self {
                val: VertexTypes::Vector3(..),
                ..
            } => DataType::F32,
            Self {
                val: VertexTypes::Vector4(..),
                ..
            } => DataType::F32,
            _ => DataType::U32,
        }
    ...
    pub fn normalized(&self) -> bool {
        match self {
            Self {
                usage: VertexUsage::Normal | VertexUsage::BlendWeight,
                val: VertexTypes::Unorm4x8(..),
            } => true,
            _ => false,
        }
    ...
    pub fn order(&self) -> usize {
        match self {
            ...
            VertexData {
                usage: VertexUsage::BlendWeight | VertexUsage::Pad(0),
                ..
            } => 1,
            VertexData {
                usage: VertexUsage::BlendIndices | VertexUsage::Pad(1),
                ..
            } => 2,
            VertexData {
                usage: VertexUsage::Pad(2),
                ..
            } => 3,
            VertexData {
                usage: VertexUsage::Pad(3),
                ..
            } => 4,
            VertexData {
                usage: VertexUsage::Pad(4),
                ..
            } => 5,
            VertexData {
                usage: VertexUsage::Normal,
            ...
            VertexData {
                usage: VertexUsage::Color(0),
                ..
            } => 7,
            VertexData {
                usage: VertexUsage::Color(1),
                ..
            } => 8,
            VertexData {
                usage: VertexUsage::TextureCoord(0),
                ..
            } => 9,
            VertexData {
                usage: VertexUsage::TextureCoord(1),
                ..
            } => 10,
            VertexData {
                usage: VertexUsage::TextureCoord(2),
                ..
            } => 11,
            VertexData {
                usage: VertexUsage::TextureCoord(3),
                ..
            } => 12,
            VertexData {
                usage: VertexUsage::Pad(5),
                ..
            } => 13,
            VertexData {
                usage: VertexUsage::Pad(6),
                ..
            } => 14,
            VertexData {
                usage: VertexUsage::Pad(7),
                ..
            } => 15,
            VertexData {
                usage: VertexUsage::Tangent,
                ..
            } => 16,
            VertexData {
                usage: VertexUsage::PSize,
                ..
            } => 17,
            _ => usize::MAX,
        }
...
impl VertexBuffer {
    ...
    pub fn from_vertex_format<V: Version>(fmt1: u32, fmt2: u32) -> Vec<VertexData> {
        ...
        if fmt2 == 0 {
            ...
            if fmt1 & 1 != 0 {
                vals.push(VertexData::new(
                    if b1 {
                        BaseTypes::VECTOR4_KEY
                    } else {
                        BaseTypes::VECTOR3_KEY
                    },
    ...
    pub fn get_vertex_format<V: Version>(&self) -> u32 {
        ...
        for val in &self.data {
            match val {
                ...
                VertexData {
                    usage: VertexUsage::Pad(0),
                    ..
                } => {
                    fmt1 |= 0x40400;
                }
                VertexData {
                    usage: VertexUsage::Pad(1),
                    ..
                } => {
                    fmt1 |= 0x40800;
                }
...
impl<'py> FromPyObject<'py> for VertexBuffer {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        ...
        let info: VBuffInfo = dict
            .get_item("info")?
            .ok_or(anyhow::anyhow!("dict missing info key"))?
            .extract()?;
        dict.del_item("info")?;
        let data: Vec<VertexData> = dict
            .into_iter()
            .map(|(k, v)| {
                Ok(VertexData {
                    usage: k.extract()?,
        ...
        if V::xbox() {
            ...
            for val in &mut vals {
                match val {
                    ...
                    VertexData {
                        usage: VertexUsage::Normal,
                        val: VertexTypes::Unorm4x8(v),
                    } => v.iter_mut().for_each(|val| {
                        let z_ = ((*val) & 0x3FF) ^ 0x200;
                        let y_ = (((*val) >> 10) & 0x3FF) ^ 0x200;
                        let x_ = (((*val) >> 20) & 0x3FF) ^ 0x200;
                        let x: u32 = (x_ as f32 - 4.0f32)
                            .div(4.0f32)
                            .round_ties_even()
                            .clamp(0.0, 255.0) as u32;
                        let y: u32 = (y_ as f32 - 4.0f32)
                            .div(4.0f32)
                            .round_ties_even()
                            .clamp(0.0, 255.0) as u32;
                        let z: u32 = (z_ as f32 - 4.0f32)
                            .div(4.0f32)
                            .round_ties_even()
                            .clamp(0.0, 255.0) as u32;
                    ...
                    VertexData {
                        usage: VertexUsage::Normal,
                        ..
                    } => panic!("Unexpected type for vertex normals"),
                    _ => (),
                }
            }
        }
    ...
    fn dump_bytes<V: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        ...
        (0..i)
            .flat_map(|i| {
                sorted_vals
                    .iter()
                    .flat_map(move |val| dump_bytes!(V, val.val.get(i)))
            })
            .collect()
    }
...
impl<'a> AsData<'a, '_> for IndexBuffer {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        if data.len() < (info.offset + info.size) as usize {
            warn!(
                "Index buffer of size {} only has data of size {} available",
                info.size,
                data.len() - info.offset as usize
            );
            n = (data.len() - info.offset as usize) / size;
        }
        Ok(match info.format {
            0x10 => Self::U16 {
                vals: from_bytes!(V, &data[info.offset as usize..], n)?,
            },
            _ => Self::U32 {
                vals: from_bytes!(V, &data[info.offset as usize..], n)?,
            },
...
impl<'a, 'b> AsData<'a, 'b> for RadiosityVals {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        Ok(Self {
            vals: from_bytes!(V, &data[info.offset as usize..], info.num as usize)?,
        })
    }
...
impl<'a> AsData<'a, '_> for Foliage {
    ...
    fn from_bytes<V: Version>(data: &[u8], info: Self::InArgs) -> Result<Self> {
        ...
        Ok(Self {
            vals: from_bytes!(V, &data[info.offset as usize..], n as usize)?,
        })
    }
