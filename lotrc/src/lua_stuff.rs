...
pub fn compile(code: &[u8], name: &str) -> Result<Vec<u8>> {
    let code = LUA
        ...
        .map_err(|e| anyhow!(e.to_string()))?;
    convert(&code.as_bytes(), &PC_FORMAT)
}
...
pub fn decomp(code: &[u8], unluac: String) -> LuaResult<String> {
    ...
    let output = std::process::Command::new("java")
        .args(&["-jar", &unluac, path.to_str().unwrap()])
        .output()?;
...
pub fn load_anim(script_fns: Arc<HashMap<Crc, Vec<u8>>>, anim: String) -> Map<String, Value> {
    let lua: Lua = Lua::new();
    let script_manager = ScriptManager {
        script_fns,
        loaded_scripts: Mutex::new(HashSet::new()),
    };
    ...
    lua.globals()
        .set(
            "import",
            lua.load("function (a) importer:import(a) end")
                .eval::<LuaFunction>()
                .unwrap(),
        )
        .unwrap();
    ...
    table
        .set(
            "GetRandomNumber",
            lua.load("function () return 1 end")
                .eval::<LuaFunction>()
                .unwrap(),
        )
        .unwrap();
    ...
    lua.globals()
        .set(
            "DeepCopy",
            lua.load("function (a) return a end")
                .eval::<LuaFunction>()
                .unwrap(),
        )
        .unwrap();
    lua.globals()
        .set(
            "AppendTableIndex",
            lua.load("function (t1, t2) for key, val in pairs(t2) do t1[key] = val end end")
                .eval::<LuaFunction>()
                .unwrap(),
        )
        .unwrap();
    lua.globals()
        .set(
            "AppendTable",
            lua.load("function (t1, t2) table.insert(t1, t2) end")
                .eval::<LuaFunction>()
                .unwrap(),
        )
        .unwrap();
    ...
    table
        .set(
            "GetRootSpeed",
            lua.load("function (a) end").eval::<LuaFunction>().unwrap(),
        )
        .unwrap();
    ...
    let tables = if lua.globals().contains_key("AnimTableUsed").unwrap() {
        lua.globals()
            .get::<LuaTable>("AnimTableUsed")
            .unwrap()
            .pairs::<LuaValue, String>()
            .filter_map(|x| x.ok())
            .map(|(_, x)| x)
            .collect()
    } else {
