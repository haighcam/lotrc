use super::{
    ...
    types::{dump_bytes, from_bytes, AsData, Crc, NoArgs, Strings, Version, PC, XBOX},
};
...
#[cfg_attr(feature = "python", pyclass(module = "shader", get_all, set_all))]
...
impl AsData<'_, '_> for Shaders {
    ...
    fn from_bytes<O: Version>(data: &[u8], _args: Self::InArgs) -> Result<Self> {
        ...
        let strings: Strings = from_bytes!(
            O,
        ...
        let vertex_headers: Vec<ShaderHeader> =
            from_bytes!(O, &data[offset..], header.vertex_shader_num as usize)?;
        offset += vertex_headers.size::<O>();
        let fragment_headers: Vec<ShaderHeader> =
            from_bytes!(O, &data[offset..], header.fragment_shader_num as usize)?;
        let fragment_offsets: Vec<_> = fragment_headers
            .iter()
            .map(|x| (x.offset + header.shaders_offset) as usize)
            .chain(std::iter::once(
                (header.shaders_size + header.shaders_offset) as usize,
            ))
            .collect();
        let vertex_offsets: Vec<_> = vertex_headers
            .iter()
            .map(|x| (x.offset + header.shaders_offset) as usize)
            .chain(std::iter::once(fragment_offsets[0]))
            .collect();
        let vertex_shaders: IndexMap<_, _> = vertex_headers
            .iter()
            .enumerate()
            .map(|(i, info)| {
                (
                    info.key.clone(),
                    data[vertex_offsets[i]..vertex_offsets[i + 1]].to_vec(),
                )
            })
            .collect();
        let fragment_shaders: IndexMap<_, _> = fragment_headers
            .iter()
            .enumerate()
            .map(|(i, info)| {
                (
                    info.key.clone(),
                    data[fragment_offsets[i]..fragment_offsets[i + 1]].to_vec(),
                )
            })
            .collect();
    ...
    fn dump_bytes<O: Version>(&self, _args: Self::OutArgs) -> Vec<u8> {
        ...
        let vertex_headers: Vec<_> = self
            .vertex_headers
            .iter()
            .zip(self.vertex_shaders.values())
            .map(|(info, shader)| {
            ...
            .collect();
        let fragment_headers: Vec<_> = self
            .fragment_headers
            .iter()
            .zip(self.fragment_shaders.values())
            .map(|(info, shader)| {
                let mut info = info.clone();
                info.offset = shaders_size;
                shaders_size += shader.len() as u32;
            ...
            .collect();
        let shaders_offset = (self.header.size::<O>()
            + vertex_headers.size::<O>()
            + fragment_headers.size::<O>()) as u32;
        ...
        dump_bytes!(O, header)
            ...
            .chain(
                vertex_headers
                    .iter()
                    .flat_map(|x| self.vertex_shaders.get(&x.key).unwrap())
                    .cloned(),
            )
            .chain(
                fragment_headers
                    .iter()
                    .flat_map(|x| self.fragment_shaders.get(&x.key).unwrap())
                    .cloned(),
            )
    ...
    fn size<V: Version>(&self) -> usize {
        self.header.size::<V>()
            ...
            + self
                .vertex_headers
                .iter()
                .map(|x| self.vertex_shaders.get(&x.key).unwrap().len())
                .sum::<usize>()
            + self
                .fragment_headers
                .iter()
                .map(|x| self.fragment_shaders.get(&x.key).unwrap().len())
                .sum::<usize>()
            + self.strings.size::<V>()
    }
}
...
impl Shaders {
    ...
    pub fn to_file(&self, writer: Writer) -> Result<()> {
        ...
        writer
            .join("vertex_headers.json")
            .write(&serde_json::to_vec_pretty(&self.vertex_headers)?)?;
        writer
            .join("fragment_headers.json")
            .write(&serde_json::to_vec_pretty(&self.fragment_headers)?)?;
    ...
    pub fn from_file(reader: Reader) -> Result<Self> {
        ...
        crate::types::update_strings(&strings.strings);
        let vertex_headers = serde_json::from_slice::<Vec<ShaderHeader>>(
            &reader.join("vertex_headers.json").read()?,
        )
        .context("vertex_headers.json")?;
        let fragment_headers = serde_json::from_slice::<Vec<ShaderHeader>>(
            &reader.join("fragment_headers.json").read()?,
        )
        .context("vertex_headers.json")?;
