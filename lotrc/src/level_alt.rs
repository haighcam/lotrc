...
use super::{
    ...
    types::{
        self, dump_bytes, from_bytes, hash_string, to_bytes, AsData, BaseTypes, CompressedBlock,
        Crc, GameObjs, SubBlock, Version, PC, PS3, XBOX, XBOXPROTO, XBOXPROTO2,
    },
};
...
#[cfg_attr(feature = "python", pyclass(module = "level_alt", set_all, get_all))]
...
impl Level {
    pub fn parse<P: AsRef<Path>>(path: P, mp: Option<&MultiProgress>) -> Result<Self> {
        ...
        let pak_data = fs::read(path.with_extension("PAK"))
            .context(path.with_extension("PAK").display().to_string())?;
        let bin_data = fs::read(path.with_extension("BIN"))
        ...
        Ok(if bin_data[0] == 6 {
            ...
            if pak_header.mat1_size == 504 {
                ...
                Self::from_data::<XBOXPROTO>(&bin_data[..], &pak_data[..], mp)?
            } else if pak_header.animation_block_info_size != 36 {
                info!("parsing prototype2");
                Self::from_data::<XBOXPROTO2>(&bin_data[..], &pak_data[..], mp)?
            } else {
    ...
    pub fn dump<O: Version + 'static, P: AsRef<Path>>(
        &mut self,
        path: P,
    ...
    pub fn from_data<O: Version + 'static>(
        ...
        let (bar, sub_bar) = if let Some(mp) = mp {
            let bar =
                ProgressBar::new(8).with_style(ProgressStyle::with_template(BAR_FMT).unwrap());
            let sub_bar =
                ProgressBar::new(0).with_style(ProgressStyle::with_template(SUB_BAR_FMT).unwrap());
            mp.add(bar.clone());
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("bin headers")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(0);
            x.reset()
        });
        let bin_header: bin::Header = from_bytes!(O, bin_data)?;
        let bin_strings = from_bytes!(
            O,
            types::Strings,
            &bin_data[bin_header.strings_offset as usize..],
            bin_header.strings_num as usize
        )?;
        ...
        sub_bar.as_ref().map(|x| {
            x.set_length(bin_header.asset_handle_num as u64);
            x.reset()
        });

        let asset_handles: Vec<bin::AssetHandle> = from_bytes!(
            O,
            &bin_data[bin_header.asset_handle_offset as usize..],
            bin_header.asset_handle_num as usize
        )?;
            ...
            asset_handles
                ...
                .map(|info| {
                    (info.key.clone(), {
                        sub_bar.as_ref().map(|x| {
                            x.inc(1);
                            x.set_message(info.key.to_string())
                        });
        ...
        let texture_data = HashMap::<Crc, Vec<u8>>::from_iter(
            asset_handles
                .iter()
                .skip(bin_header.vdata_num as usize)
                .map(|info| {
                    sub_bar.as_ref().map(|x| {
                        x.inc(1);
                        x.set_message(info.key.to_string())
        ...
        let pak_strings = from_bytes!(
            O,
            types::Strings,
            &pak_data[pak_header.strings_offset as usize..],
            pak_header.strings_num as usize
        )?;
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("block2")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(0);
            x.reset()
        });
        let block2 = types::CompressedBlock::from_data::<O>(
            pak_data,
            pak_header.block2_size as usize,
            pak_header.block2_size_comp as usize,
            pak_header.block2_offset as usize,
        )
        .data;
        let mut sub_blocks2 = from_bytes!(
            O,
            types::SubBlocks,
            &block2[pak_header.sub_blocks2_offset as usize..],
            sub_bar.clone().into()
        )?;
        let block2_offsets = from_bytes!(
            O,
            &block2[pak_header.block2_offsets_offset as usize..],
        ...
        sub_bar.as_ref().map(|x| {
            x.set_length(
                (pak_header.animation_block_info_num
                    ...
                    + pak_header.animation_info_num) as u64,
            );
            x.reset()
        });
        let block1 = types::CompressedBlock::from_data::<O>(
            pak_data,
            pak_header.block1_size as usize,
            pak_header.block1_size_comp as usize,
            pak_header.block1_offset as usize,
        )
        .data;
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("sub blocks")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(0);
            x.reset()
        });
        let sub_blocks1 = from_bytes!(
            O,
            types::SubBlocks,
            &block1[pak_header.sub_blocks1_offset as usize..],
            sub_bar.clone().into()
        )?;
        let string_keys = from_bytes!(
            O,
            types::StringKeys,
            &block1[pak_header.string_keys_offset as usize..]
        )?;
        ...
        let objas = from_bytes!(
            O,
            &block1[pak_header.obja_offset as usize..],
            pak_header.obja_num as usize
        )?;
        let obj0s = from_bytes!(
            O,
            &block1[pak_header.obj0_offset as usize..],
            pak_header.obj0_num as usize
        )?;
        let animation_block_infos: Vec<pak::AnimationBlockInfo> = from_bytes!(
            O,
            &block1[pak_header.animation_block_info_offset as usize..],
            pak_header.animation_block_info_num as usize
        )?;
        let pfield_infos = from_bytes!(
            O,
            &block1[pak_header.pfield_info_offset as usize..],
            pak_header.pfield_info_num as usize
        )?;
        if let Some(types::SubBlock::PFields(val)) =
            sub_blocks2.blocks.get_mut(&Crc::Key(types::PFields::KEY))
        {
            val.parse(pfield_infos);
        }
        info!("pfield_infos parsed in {:?}", time.elapsed());

        let models = from_bytes!(
            O,
            Vec<pak::ModelInfo>,
            &block1[pak_header.model_info_offset as usize..],
        ...
        info!("models parsed in {:?}", time.elapsed());

        let effects = from_bytes!(
            O,
            Vec<pak::EffectInfo>,
            &block1[pak_header.effect_info_offset as usize..],
        ...
        .map(|info| {
            ...
            Ok((
                ...
                from_bytes!(
                    O,
                    GameObjs,
                    &block1[info.offset as usize..],
                    info.gamemodemask
                )?,
            ))
        })
        .collect::<Result<IndexMap<_, _>>>()?;
        info!("effects parsed in {:?}", time.elapsed());

        let gfx_blocks = from_bytes!(
            O,
            Vec<pak::GFXBlockInfo>,
            &block1[pak_header.gfx_block_info_offset as usize..],
        ...
        .map(|info| {
            ...
            (
                info.key,
                block1[info.offset as usize..(info.offset + info.size) as usize].to_vec(),
            )
        })
        .collect::<IndexMap<_, _>>();
        info!("gfx_blocks parsed in {:?}", time.elapsed());

        let mut foliages: IndexMap<Crc, Vec<(pak::FoliageInfo, pak::Foliage)>> =
            IndexMap::with_capacity(pak_header.foliage_info_num as usize);
        for info in from_bytes!(
            O,
            Vec<pak::FoliageInfo>,
            &block1[pak_header.foliage_info_offset as usize..],
        ...
        info!("foliages parsed in {:?}", time.elapsed());

        let textures = from_bytes!(
            O,
            Vec<pak::TextureInfo>,
            &block1[pak_header.texture_info_offset as usize..],
        ...
        .map(|mut info| {
            ...
            let data0 = texture_data.get(&info.asset_key).expect(
                format!("could not find texture data {}", info.asset_key.to_string()).as_str(),
            );
            let data1 = texture_data
                .get(&Crc::Key(hash_string(
                    "*".as_bytes(),
                    Some(info.asset_key.key()),
                )))
                .unwrap();
        ...
        .collect::<Result<IndexMap<_, _>>>()?;
        info!("textures parsed in {:?}", time.elapsed());

        let animations = if O::xbox_proto() {
            let data = match sub_blocks2
                .blocks
            ...
            let mut offset = 0;
            let animations = from_bytes!(
                O,
                Vec<pak::AnimationInfo>,
                &block1[pak_header.animation_info_offset as usize..],
            ...
            .map(|info| {
                ...
                let anim = Animation::from_data_alt::<O>(info, &mut offset, data)?;
                Ok((anim.info.key.clone(), anim))
            })
            .collect::<Result<IndexMap<_, _>>>()?;
            animations
        } else {
            let blocks = animation_block_infos
                .iter()
                .map(|info| {
                    sub_bar.as_ref().map(|x| {
                        x.inc(1);
        ...
        info!("animations parsed in {:?}", time.elapsed());

        let gameobjs =
            if let Some(SubBlock::GameObjs(objs)) = sub_blocks1.blocks.get(&Crc::Key(2083108783)) {
                Some(objs)
            } else {
                None
            }
            .unwrap();
        let level_name = if let Some(BaseTypes::CRC(val)) = gameobjs
            .objs
            .first()
            .unwrap()
            .1
            .fields
            .get(&Crc::Key(2970763744))
        {
            Some(val.clone())
        } else {
            None
        ...
        .unwrap();
        let infos = from_bytes!(
            O,
            Vec<pak::RadiosityValsInfo>,
            &block1[pak_header.radiosity_vals_info_offset as usize..],
        ...
        let radiosity = if let Some(rad_data) = model_data.get(&rad_name) {
            let rad_usage = asset_handles
                .iter()
                .find(|x| x.key == rad_name)
                .unwrap()
                .kind;
            Some(from_bytes!(
                O,
        ...
        info!("radiosity parsed in {:?}", time.elapsed());
        ...
        let pak_vals_a = from_bytes!(
            O,
            &pak_data[pak_header.block_a_offset as usize..],
            pak_header.block_a_num as usize
        )?;
    ...
    pub fn to_data<O: Version + 'static>(
        &mut self,
        mp: Option<&MultiProgress>,
    ) -> Result<(Vec<u8>, Vec<u8>, DumpInfos)> {
        let time = Instant::now();
        info!("compressing level");

        let (bar, sub_bar) = if let Some(mp) = mp {
            let bar =
                ProgressBar::new(9).with_style(ProgressStyle::with_template(BAR_FMT).unwrap());
            let sub_bar =
                ProgressBar::new(0).with_style(ProgressStyle::with_template(SUB_BAR_FMT).unwrap());
            mp.add(bar.clone());
            mp.add(sub_bar.clone());
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("textures")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(self.textures.len() as u64);
            x.reset()
        });
        ...
        let texture_infos = self
            ...
            .map(|tex| {
                sub_bar.as_ref().map(|x| {
                    x.inc(1);
                    x.set_message(tex.info().key.to_string())
                });
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("animations")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length((self.animation_block_infos.len() + self.animations.len()) as u64);
            x.reset()
        });

        // pak stuff
        let mut pak_header = self.pak_header.clone();
        pak_header.version = if O::pc() {
            1
        } else if O::xbox() {
            2
        } else if O::ps3() {
        ...
        let pfield_infos = if let Some(types::SubBlock::PFields(val)) =
            self.sub_blocks2.blocks.get(&Crc::Key(types::PFields::KEY))
        {
            val.infos()
        } else {
            vec![]
        ...
        block1.extend(vec![
            ...
            pak_header.buffer_info_num as usize
                * O::size::<pak::BufferInfo>()
        ]);
        block1.extend(vec![0u8; ((block1.len() + 15) & 0xFFFFFFF0) - block1.len()]);
        pak_header.mat1_offset = block1.len() as u32;
        block1.extend(vec![
            0u8;
            pak_header.mat1_num as usize * O::size::<pak::MatBase>()
        ]);
        block1.extend(vec![0u8; ((block1.len() + 15) & 0xFFFFFFF0) - block1.len()]);
        pak_header.mat2_offset = block1.len() as u32;
        block1.extend(vec![
            0u8;
            pak_header.mat2_num as usize * O::size::<pak::Mat2>()
        ]);
        block1.extend(vec![0u8; ((block1.len() + 15) & 0xFFFFFFF0) - block1.len()]);
        pak_header.mat3_offset = block1.len() as u32;
        block1.extend(vec![
            0u8;
            pak_header.mat3_num as usize * O::size::<pak::Mat3>()
        ]);
        block1.extend(vec![0u8; ((block1.len() + 15) & 0xFFFFFFF0) - block1.len()]);
        pak_header.mat4_offset = block1.len() as u32;
        block1.extend(vec![
            0u8;
            pak_header.mat4_num as usize * O::size::<pak::Mat4>()
        ]);
        block1.extend(vec![0u8; ((block1.len() + 15) & 0xFFFFFFF0) - block1.len()]);
        ...
        // infos done
        let mut offset = 0;
        let animation_vals = self
            .animations
            .iter()
            .sorted_by(|a, b| a.0.key().cmp(&b.0.key()))
            .map(|(key, anim)| {
                sub_bar.as_ref().map(|x| {
                    x.inc(1);
                    x.set_message(key.to_string())
                });
            ...
            .collect::<Vec<_>>();
        let animations_blocks = (0..self.animation_block_infos.len() as u32)
            .map(|i| {
                let gamemodemask = 1i32 << i;
                animation_vals
                    .iter()
                    .filter(|(_, k)| k & gamemodemask != 0)
                    .flat_map(|(x, _)| x)
                    .cloned()
                    .collect::<Vec<_>>()
            })
        ...
        for (info, data) in zip(&mut animation_block_infos, animations_blocks) {
            ...
            pak_data.extend(vec![
                0u8;
                ((pak_data.len() + 4095) & 0xfffff000) - pak_data.len()
            ]);
            let size = data.len();
            let data = CompressedBlock { data }
                .dump(true)
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("effects")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(self.effects.len() as u64);
            x.reset()
        });
        ...
        let effects = self
            ...
            .map(|(key, effect)| {
                ...
                let effect = pak::EffectInfo {
                    key: key.clone(),
                    gamemodemask: effect.gamemodemask,
                    offset: block1.len() as u32,
                    size: vals.len() as u32,
                };
                block1.extend(vals);
                Ok(effect)
            })
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("models & foliages")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length((self.models.len() + self.foliages.len()) as u64);
            x.reset()
        });
        ...
        terrain.sort_unstable_by_key(|x| {
            x.str()
                .and_then(|x| x.split('_').last().and_then(|x| x.parse::<usize>().ok()))
                .unwrap_or_default()
        });
        if let Some(SubBlock::GameObjs(objs)) = self.sub_blocks1.blocks.get(&Crc::Key(2083108783)) {
            collision_road.sort_unstable_by_key(|x| {
                objs.objs.get_index_of(
                    &x.str()
                        .and_then(|x| x.split('_').last().and_then(|x| x.parse::<u32>().ok()))
                        .unwrap_or_default(),
                )
            });
        } else {
        ...
        for model in normal
            .into_iter()
            .chain(collision_road)
            .map(|key| self.models.get(key).unwrap())
        {
            sub_bar.as_ref().map(|x| {
                x.inc(1);
                x.set_message(model.info.key.to_string())
            });
            let (vals, mesh_vals) = model.dump::<O>(block1.len(), &mut infos);
            model_data.push((
                (model.info.asset_key.clone(), model.info.asset_type),
                mesh_vals,
            ));
            block1.extend(vals);
            block1.extend(vec![0u8; ((block1.len() + 15) & 0xFFFFFFF0) - block1.len()]);
        ...
        for model in terrain.into_iter().map(|key| self.models.get(key).unwrap()) {
            ...
            let (vals, mesh_vals) =
                model.dump_terrain::<O>(block1.len(), terrain_start_offset, &mut infos);
            model_data.push((
                (model.info.asset_key.clone(), model.info.asset_type),
                mesh_vals,
            ));
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("block1 objs")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length((self.gfx_blocks.len() + 1) as u64);
            x.reset()
        });
        ...
        let gfx_blocks = self
            ...
            .map(|(key, val)| {
                ...
                let gfx_block = pak::GFXBlockInfo {
                    key: key.clone(),
                    offset: block1.len() as u32,
                    size: val.len() as u32,
                };
        ...
        let gameobjs = if let Some(SubBlock::GameObjs(objs)) =
            self.sub_blocks1.blocks.get(&Crc::Key(2083108783))
        {
            Some(objs)
        } else {
            None
        ...
        to_bytes!(
            O,
            self.objas,
            &mut block1[pak_header.obja_offset as usize..]
        )?;
        to_bytes!(
            O,
            self.obj0s,
            &mut block1[pak_header.obj0_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.model,
            &mut block1[pak_header.model_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.buffer,
            &mut block1[pak_header.buffer_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.mat1,
            &mut block1[pak_header.mat1_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.mat2,
            &mut block1[pak_header.mat2_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.mat3,
            &mut block1[pak_header.mat3_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.mat4,
            &mut block1[pak_header.mat4_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.mat_extra,
            &mut block1[pak_header.mat_extra_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.shape,
            &mut block1[pak_header.shape_info_offset as usize..]
        )?;
        for (i, hk_shape) in infos.hk_shape.iter().enumerate() {
            to_bytes!(
                O,
                hk_shape,
                &mut block1[pak_header.hk_shape_info_offset as usize + i * O::size::<HkShape0>()..]
            )?;
        }
        to_bytes!(
            O,
            infos.hk_constraint_data,
            &mut block1[pak_header.hk_constraint_data_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.vbuff,
            &mut block1[pak_header.vbuff_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.ibuff,
            &mut block1[pak_header.ibuff_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            texture_infos,
            &mut block1[pak_header.texture_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.animation,
            &mut block1[pak_header.animation_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.hk_constraint,
            &mut block1[pak_header.hk_constraint_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            effects,
            &mut block1[pak_header.effect_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            foliages,
            &mut block1[pak_header.foliage_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            pfield_infos,
            &mut block1[pak_header.pfield_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            gfx_blocks,
            &mut block1[pak_header.gfx_block_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            infos.radiosity_vals,
            &mut block1[pak_header.radiosity_vals_info_offset as usize..]
        )?;
        to_bytes!(
            O,
            animation_block_infos,
            &mut block1[pak_header.animation_block_info_offset as usize..]
        )?;
        ...
        for (i, model) in infos.model.iter().enumerate() {
            ...
            if model.bones_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 136,
                );
            }
            if model.skin_order_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 160,
                );
            }
            if model.vals_j_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 196,
                );
            }
            if model.block_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 200,
                );
            }
            if model.vals_k_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 204,
                );
            }
            if model.shape_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 224,
                );
            }
            if model.hk_constraint_data_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 232,
                );
            }
            if model.hk_constraint_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 240,
                );
            }
            if model.slots_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 244,
                );
            }
            if model.slot_map_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.model_info_offset + (i * O::size::<pak::ModelInfo>()) as u32 + 248,
                );
            }
        }
        for (i, buffer) in infos.buffer.iter().enumerate() {
            ...
            if buffer.vbuff_info_offset_2 != 0 {
                infos.block2_offsets.push(
                    pak_header.buffer_info_offset + (i * O::size::<pak::BufferInfo>()) as u32 + 4,
                );
            }
            if buffer.vbuff_info_offset_3 != 0 {
                infos.block2_offsets.push(
                    pak_header.buffer_info_offset + (i * O::size::<pak::BufferInfo>()) as u32 + 8,
                );
            }
        }
        for (i, mat) in infos.mat1.iter().enumerate() {
            if mat.base.mat_extra_offset != 0 {
                infos
                    .block2_offsets
                    .push(pak_header.mat1_offset + (i * O::size::<pak::MatBase>()) as u32 + 344);
            }
        }
        for (i, mat) in infos.mat2.iter().enumerate() {
            if mat.base.mat_extra_offset != 0 {
                infos
                    .block2_offsets
                    .push(pak_header.mat2_offset + (i * O::size::<pak::Mat2>()) as u32 + 344);
            }
        }
        for (i, mat) in infos.mat3.iter().enumerate() {
            if mat.base.mat_extra_offset != 0 {
                infos
                    .block2_offsets
                    .push(pak_header.mat3_offset + (i * O::size::<pak::Mat3>()) as u32 + 344);
            }
        }
        for (i, mat) in infos.mat4.iter().enumerate() {
            if mat.base.mat_extra_offset != 0 {
                infos
                    .block2_offsets
                    .push(pak_header.mat4_offset + (i * O::size::<pak::Mat4>()) as u32 + 344);
            }
        }
        for (i, shape) in infos.shape.iter().enumerate() {
            if shape.hk_shape_offset != 0 {
                infos.block2_offsets.push(
                    pak_header.shape_info_offset + (i * O::size::<pak::ShapeInfo>()) as u32 + 112,
                );
            }
        }
        for (i, hk_shape) in infos.hk_shape.iter().enumerate() {
            infos.block2_offsets.extend(match hk_shape {
                HkShapeInfo::ConvexVertices(_) => vec![
                    pak_header.hk_shape_info_offset
                        + (i * O::size::<pak::HkShapeInfo>()) as u32
                        + 44,
                    pak_header.hk_shape_info_offset
                        + (i * O::size::<pak::HkShapeInfo>()) as u32
                        + 52,
        ...
        for (i, hk_constraint) in infos.hk_constraint.iter().enumerate() {
            infos.block2_offsets.extend([
                pak_header.hk_constraint_info_offset
                    + (i * O::size::<pak::HkConstraintInfo>()) as u32
                    + 4,
                pak_header.hk_constraint_info_offset
                    + (i * O::size::<pak::HkConstraintInfo>()) as u32
                    + 12,
        ...
        for i in 0..pak_header.effect_info_num as usize {
            infos.block2_offsets.push(
                pak_header.effect_info_offset + (i * O::size::<pak::EffectInfo>()) as u32 + 8,
            );
        }
        for i in 0..pak_header.gfx_block_info_num as usize {
            infos.block2_offsets.push(
                pak_header.gfx_block_info_offset + (i * O::size::<pak::GFXBlockInfo>()) as u32 + 4,
            );
        }
        for i in 0..pak_header.radiosity_vals_info_num as usize {
            infos.block2_offsets.push(
                pak_header.radiosity_vals_info_offset
                    + (i * O::size::<pak::RadiosityValsInfo>()) as u32
                    + 8,
            );
        }
        for i in 0..pak_header.foliage_info_num as usize {
            infos.block2_offsets.push(
                pak_header.foliage_info_offset + (i * O::size::<pak::FoliageInfo>()) as u32 + 28,
            );
        }
        ...
        let size = block1.len();
        let data = CompressedBlock { data: block1 }
            .dump(true)
            .context("pak_block1")?;
        ...
        let size = block2.len();
        let data = CompressedBlock { data: block2 }
            .dump(true)
            .context("pak_block2")?;
        ...
        pak_data.extend(vec![
            0u8;
            ((pak_data.len() + 2047) & 0xfffff800) - pak_data.len()
        ]);
        ...
        sub_bar.as_ref().map(|x| {
            x.set_length(
                (model_data.len()
                    + texture_data.len()
                    + 2
                    + rad_data.is_some().then_some(1).unwrap_or(0)) as u64,
            );
            x.reset()
        });
        ...
        bin_data.extend(vec![
            0u8;
            ((bin_data.len() + 2047) & 0xfffff800) - bin_data.len()
        ]);
        let mut model_asset_handles = model_data
            .into_iter()
            .map(|((key, kind), data)| {
                sub_bar.as_ref().map(|x| {
                    x.inc(1);
                ...
                let size_comp = if size != 0 {
                    let data = CompressedBlock { data }
                        .dump(false)
                        .with_context(|| format!("{}", key.to_string()))?;
        ...
        let mut texture_asset_handles = texture_data
            .into_iter()
            .map(|((key, kind), data)| {
                sub_bar.as_ref().map(|x| {
                    x.inc(1);
        ...
        if let Some(rad_data) = rad_data {
            sub_bar.as_ref().map(|x| {
                x.inc(1);
                x.set_message(rad_name.to_string())
            });
            let size = rad_data.len() as u32;
            let offset = bin_data.len() as u32;
            let size_comp = if size != 0 {
                let data = CompressedBlock { data: rad_data }
                    .dump(false)
                    .with_context(|| format!("{}", rad_name.to_string()))?;
                let size_comp = data.len() as u32;
                bin_data.extend(data);
                bin_data.extend(vec![
                    0u8;
        ...
        for (i, animation_block_info) in animation_block_infos.iter().enumerate() {
            ...
            for model in self.models.values() {
                if (model.info.gamemodemask & gamemodemask) != 0 {
                    for asset_handle in &asset_handles {
                        if asset_handle.key == model.info.asset_key
                            && asset_handle.kind == model.info.asset_type
                        {
            ...
            info!(
                "Gamemode {} min buffer sizes: texture {}, vertex {}",
                animation_block_info.key.to_string(),
                tex_size,
                vert_size
            );
        }
    ...
    pub fn to_file(&self, writer: Writer, mp: Option<MultiProgress>) -> Result<()> {
        ...
        let (bar, sub_bar) = if let Some(mp) = mp {
            let bar =
                ProgressBar::new(11 + types::ANIM_TABLES.lock().unwrap().then_some(1).unwrap_or(0))
                    .with_style(ProgressStyle::with_template(BAR_FMT).unwrap());
            let sub_bar =
                ProgressBar::new(0).with_style(ProgressStyle::with_template(SUB_BAR_FMT).unwrap());
            mp.add(bar.clone());
            mp.add(sub_bar.clone());
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("headers")
        });
        writer
            .join("bin_header.json")
            .write(&to_vec_pretty(&self.bin_header)?)?;
        self.bin_strings.to_file(writer.join("bin_strings"))?;
        writer
            .join("pak_header.json")
            .write(&to_vec_pretty(&self.pak_header)?)?;
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("unused objs")
        });
        writer
            .join("objas.json")
            .write(&to_vec_pretty(&self.objas)?)?;
        writer
            .join("obj0s.json")
            .write(&to_vec_pretty(&self.obj0s)?)?;
        writer
            .join("pak_vals_a.json")
            .write(&to_vec_pretty(&self.pak_vals_a)?)?;
        info!("unused objs in {:?}", time.elapsed());

        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("models")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(self.models.len() as u64);
            x.reset()
        });
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("foliage objs")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(self.foliages.len() as u64);
            x.reset()
        });
        ...
        sub_bar.as_ref().map(|x| {
            x.set_length(self.gfx_blocks.len() as u64);
            x.reset()
        });
        ...
        sub_bar.as_ref().map(|x| {
            x.set_length(self.animations.len() as u64);
            x.reset()
        });

        writer
            .join("animation_block_infos.json")
            .write(&to_vec_pretty(&self.animation_block_infos)?)?;
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("radiosity")
        });
        if let Some(radiosity) = self.radiosity.as_ref() {
            writer
                .join("radiosity.json")
                .write(&to_vec_pretty(radiosity)?)?;
        }
        info!("radiosity in {:?}", time.elapsed());

        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("sub_blocks1")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(0);
            x.reset()
        });
        self.sub_blocks1.to_file(
            writer.join("sub_blocks1"),
            &self.string_keys,
            sub_bar.as_ref(),
        )?;
        ...
        bar.as_ref().map(|x| {
            x.inc(1);
            x.set_message("sub_blocks2")
        });
        sub_bar.as_ref().map(|x| {
            x.set_length(0);
            x.reset()
        });
        self.sub_blocks2.to_file(
            writer.join("sub_blocks2"),
            &self.string_keys,
            sub_bar.as_ref(),
        )?;
        ...
        if *types::ANIM_TABLES.lock().unwrap() {
            bar.as_ref().map(|x| {
                x.inc(1);
                x.set_message("animation tables")
            });
            sub_bar.as_ref().map(|x| {
                x.set_length(self.sub_blocks1.blocks.len() as u64 * 2);
                x.reset()
            });
            let mut script_manager = HashMap::new();
            {
                for (key, block) in &self.sub_blocks1.blocks {
                    ...
                    if let types::SubBlock::Lua(val) = block {
                        ...
                        script_manager.insert(
                            Crc::from_string(&name),
                            val.conv(&crate::lua_stuff::TOOL_FORMAT)?,
                        );
                    }
                }
            }

            let anim_scripts = script_manager
                .keys()
                .filter_map(|x| {
                    x.str()
                        .and_then(|x| x.starts_with("ANM_").then_some(x.to_string()))
                })
                .collect::<Vec<_>>();
            sub_bar.as_ref().map(|x| {
                x.set_length(
                    x.length().unwrap() - self.sub_blocks1.blocks.len() as u64
                        + anim_scripts.len() as u64,
                )
            });
            ...
            for anim in anim_scripts.into_iter() {
                sub_bar.as_ref().map(|x| {
                    x.inc(1);
                    x.set_message(anim.clone())
                });
                let val = lua_stuff::load_anim(script_manager.clone(), anim.clone());
                writer
                    .join("animation_tables")
                    .join(anim)
                    .with_extension("json")
                    .write(&to_vec_pretty(&val)?)?;
            }
    ...
    pub fn from_file(reader: Reader, mp: Option<&MultiProgress>) -> Result<Self> {
        ...
        let (bar, sub_bar) = if let Some(mp) = mp {
            let bar =
                ProgressBar::new(11).with_style(ProgressStyle::with_template(BAR_FMT).unwrap());
            let sub_bar =
                ProgressBar::new(0).with_style(ProgressStyle::with_template(SUB_BAR_FMT).unwrap());
            mp.add(bar.clone());
            mp.add(sub_bar.clone());
        ...
        let bin_header =
            serde_json::from_slice::<bin::Header>(&reader.join("bin_header.json").read()?)
                .context("bin_header")?;
        let bin_strings = types::Strings::from_file(reader.join("bin_strings"))?;
        types::update_strings(&bin_strings.strings);

        let pak_header =
            serde_json::from_slice::<pak::Header>(&reader.join("pak_header.json").read()?)
                .context("pak_header")?;
        ...
        let obj0s = serde_json::from_slice::<Vec<pak::Obj0>>(&reader.join("obj0s.json").read()?)
            .context("obj0s")?;
        let pak_vals_a =
            serde_json::from_slice::<Vec<pak::BlockAVal>>(&reader.join("pak_vals_a.json").read()?)
                .context("pak_vals_a")?;
        info!("unused objs in {:?}", time.elapsed());
            ...
            if let Some(data) = match path.path().extension().and_then(|x| x.to_str()) {
                Some("glb") => {
                    ...
                    let root = gltf::json::Root::from_slice(&glb.json)
                        .with_context(|| format!("{:?}", path.path().display()))?;
                    Some(
                        Model::from_gltf(&root, bin)
                            .with_context(|| format!("{:?}", path.path().display()))?,
                    )
                }
                Some("json") => Some(
                    serde_json::from_slice::<Model>(&path.read()?)
                        .with_context(|| format!("{:?}", path.path().display()))?,
                ),
                _ => None,
            } {
        ...
        for path in data {
            ...
            let data =
                serde_json::from_slice::<Vec<(pak::FoliageInfo, pak::Foliage)>>(&path.read()?)
                    .with_context(|| format!("{:?}", path.path().display()))?;
        ...
        let animation_block_infos = serde_json::from_slice::<Vec<pak::AnimationBlockInfo>>(
            &reader.join("animation_block_infos.json").read()?,
        )?;
        ...
        for path in
            data.filter(|x| x.path().extension().unwrap_or(OsStr::new("")).to_str() == Some("json"))
        {
            let key = Crc::from_string(path.name());
            sub_bar.as_ref().map(|x| {
                x.inc(1);
        ...
        let radiosity = if rad_path.is_file() {
            Some(
                serde_json::from_slice::<Radiosity>(&reader.join("radiosity.json").read()?)
                    .context("radiosity")?,
            )
        } else {
            None
        ...
        let sub_blocks1 = types::SubBlocks::from_file(
            reader.join("sub_blocks1"),
            &string_keys,
            sub_bar.as_ref(),
        )?;
        ...
        let sub_blocks2 = types::SubBlocks::from_file(
            reader.join("sub_blocks2"),
            &string_keys,
            sub_bar.as_ref(),
        )?;
